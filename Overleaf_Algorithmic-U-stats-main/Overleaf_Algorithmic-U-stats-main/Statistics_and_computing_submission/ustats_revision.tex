...
\usepackage{makecell}
\usepackage{cancel}
\usepackage[normalem]{ulem} % \sout for the \del markup (normalem keeps \emph italic)
...
% deleted text: red strikethrough while changes are shown, vanishes entirely
% under \showchangesfalse (inline text only -- \sout cannot strike display math)
\newcommand{\del}[1]{%
  \ifshowchanges
    {\color{red}\sout{#1}}%
  \fi
...
\if1\anon
{
 ...
 \author{
    \centering
    Xingyu Chen\orcidlink{0009-0008-0823-4406}\footnote{Email: \href{xingyuchen0714@sjtu.edu.cn}{xingyuchen0714@sjtu.edu.cn}} \\
    School of Mathematical Sciences, Shanghai Jiao Tong University \\
    Lin Liu\orcidlink{0000-0002-9883-7962}\footnote{Email: \href{linliu@sjtu.edu.cn}{linliu@sjtu.edu.cn}} \\
    Institute of Natural Sciences, MOE-LSC, \\ 
    School of Mathematical Sciences, CMA-Shanghai, SJTU-Yale Joint Center for Biostatistics and Data Science, Shanghai Jiao Tong University \\
    Ruiqi Zhang\footnote{Email: \href{}{}. All authors are alphabetically ordered. The authors thank the two anonymous referees and the associate editor for insightful comments that help improve the quality of our paper. The authors also benefit from discussions with Shan Luo, Rajarshi Mukherjee, Yixuan Qiu, Jamie Robins, Xinwei Shen, and Qingyuan Zhao. The authors are supported by National Key R\&D Program of China Project Number 2025YFA1016700, NSFC Grant No. 12471274 and Science and Technology Talent and Platform Program of Yunnan Province Grant No.202605AF35007.} \\
...
\label{def:tensors}
Given positive integers $m, n > 0$, an $m$-th order tensor (i.e. an $m$-dimensional array) $T$ of size $n$ is defined as a map from $[n]^m$ to $\reals$: for any $\alpha \in [n]^{m}$, $T (\alpha) \in \bbR$. Here we can safely assume that each dimension of a tensor has the same size (i.e. number of entries in each dimension) unless explicitly mentioned otherwise.
\end{definition}

\rev{To distinguish tensors from general functions, we view the evaluation of $T$ at a given index $\alpha$ as an oracle query, and hence ignore the evaluation cost. More precisely, accessing an entry $T(\alpha)$ of an explicitly represented tensor is treated as direct data access. In this paper, we measure computational complexity by counting floating-point operations, such as additions and multiplications of floating-point numbers. Entry-access costs are therefore not included in our floating-point operation complexity analysis.}

In statistical applications, $\bbX$ and the tuple $\bm{X}$, respectively, correspond to the sample space and the observed data sample of size $n$. Unlike $U$-statistics, computing $V$-statistics amounts to summing over all $[n]^{m}$ without being restricted to permutations. As will be clear later in this paper, $V$-statistics can be represented as summations over higher-order tensors. This unique property renders computing $V$-statistics, but not $U$-statistics, amenable for tensor operations such as the Einstein summation. In particular, if the kernel function $h$ satisfies certain decomposable structure (see Definition~\ref{def:mul_decomp} later), the computational complexity of $V$-statistics can be notably reduced. Our algorithmic framework is thus to first turn a $U$-statistic into a linear combination of $V$-statistics and then compute each individual $V$-statistic via the Einstein summation.
...
\rev{Example~\ref{eg:running} below which will also appear in Section~\ref{sec:alg_u} illustrates the case $B = \emptyset$, in which the output of the \Einsum{} operation is a scalar; this is the case mainly used in the main text, since the $V$-statistics under consideration are all output a scalar.

The general case $B \neq \emptyset$, in which the output is a tensor, is nevertheless indispensable for the  analysis of computation complexity: the proof of Proposition~\ref{pro:complexity_einsum} in Appendix~\ref{app:complexity_einsum} computes a $V$-statistic by summing over one index at a time, and each intermediate step of this procedure is itself an \Einsum{} operation with a non-empty output tuple. Two further examples with $B \neq \emptyset$ are provided in Appendix~\ref{app:examples}.}

\begin{example}
\label{eg:running}
The \rev{running} example that we consider in the main text will also be used in Section~\ref{sec:alg_u_v} at various stages. Given three second-order tensors (matrices) $\calT = (T_1, T_2, T_3)$, suppose we are interested in computing a scalar $T^{\rm out}$:
\begin{align*}
...
Let $\samplespace$ be a non-empty set, $m$ a positive integer, and $h: \samplespace^m \to \bbR$. Suppose $\calA \coloneqq (A_k)_{k=1}^K$, \rev{ with $A_k \in [m]^{*}, \forall k \in [K]$ satisfies $\bigcup_{k=1}^K \takesetp{A_k} = [m]$ }, and let $\calH \coloneqq (h_k)_{k=1}^K$ be a tuple of functions where each $h_k$ is defined on $\samplespace^{|A_k|}$. $\calH \times \calA$, consisting of ordered pairs of the form $(h_{k}, A_{k})$ for $k \in [K]$, is called a multiplicative decomposition of the function $h$, if for any $\bm{x} \in \samplespace^m$, 
\begin{align*}
...
\begin{remark}\label{rem:complexity_tensorization}
\rev{
We now discuss the computational cost of Algorithm~\ref{alg:tensorization}. Throughout this paper, we assume that evaluating each factor $h_k$ of the kernel on a single tuple of samples has cost independent of the sample size $n$ and the order $m$ of the $U$-/ $V$-statistic. Hence, evaluating $h_k$ over all possible combinations of the samples $\{X_i\}_{i=1}^n$ requires $O(n^{|A_k|})$ operations, where $|A_k|$ is the number of arguments of $h_k$. 
Therefore, the total complexity of Algorithm~\ref{alg:tensorization} is
\begin{equation*}
    O\Bigg(\sum_{k=1}^K n^{|A_k|}\Bigg) 
    = O\Big(K n^{m_{\max}}\Big),
\end{equation*}
where $m_{\max} = \max_{k\in[K]} |A_k|$.
In particular, when $K$ is fixed, this complexity is $O(n^{m_{\max}})$.

Moreover, under the decomposition graph associated with the \Einsum{} notation in Definition~\ref{def:graph_decomp}, each factor $h_k$ corresponds to a clique of $|A_k|$ vertices. Therefore, according to the theory of the treewidth, $|A_k| \leq \treewidth(G) +1$, where $tw(G)$ denotes the treewidth of the decomposition graph $G$. And it follows that $m_{\max{}} \leq \treewidth(G) +1$. According to Proposition~\ref{pro:complexity_einsum}, the complexity of Algorithm~\ref{alg:tensorization} would not dominate the complexity of computing $U$-/$V$-statistics. 
}
\end{remark}
...
\rev{The proof of Proposition~\ref{pro:complexity_einsum}, deferred to Appendix~\ref{app:complexity_einsum}, rests on two elementary observations. First, summing an \Einsum{} expression over a single index $x$ costs $\less(|\calA|\, n^{\degp{x}{G_{\calA}} + 1})$ time and $n^{\degp{x}{G_{\calA}}}$ additional storage: the result of the partial summation is an intermediate tensor indexed by the neighbors of $x$ in the decomposition graph, hence with $n^{\degp{x}{G_{\calA}}}$ entries, each a sum of $n$ terms, each term a product of the (at most $|\calA|$) tensors involving $x$. Second, this partial summation acts on the decomposition graph exactly as the vertex elimination of Definition~\ref{def:treewidth_elimination}: $x$ disappears, and its neighbors become mutually connected through the new intermediate tensor. }

\rev{Figure~\ref{fig:elimination_sketch} visualizes the correspondence on the \Einsum{} notation $\calA = ((1,2), (2,1), (1,3))$, whose decomposition graph (Figure~\ref{fig:graph_hoif_4_2}) is a path centered at vertex $1$. The two branches show why the ordering matters: eliminating the central vertex first costs $\less(n^{3})$, whereas the ordering $\sigma = (2, 1, 3)$ never eliminates a vertex at degree above $1$ and completes the computation in $\less(n^{2})$. No ordering can do better, as the first vertex eliminated always has positive degree; hence $\treewidthp{G_{\calA}} = 1$, and the optimal exponent $\treewidthp{G_{\calA}} + 1 = 2$ is exactly that asserted by Proposition~\ref{pro:complexity_einsum}.}
...
\begin{tikzpicture}[
  ...
  state/.style={draw, rounded corners, align=center, inner sep=7pt},
  note/.style={align=center, font=\small},
  arrow/.style={-{Latex}, thick},
  point/.style={circle, draw, minimum size=5mm, inner sep=0pt, font=\scriptsize}
]
% ================= root: the einsum of Figure (b) =================
\node[state] (root) at (0, 0) {%
  $\displaystyle \sum_{{\color{red}i_1},\, {\color{blue}i_2},\, i_3 \,\in\, [n]}
     T_1({\color{red}i_1}, {\color{blue}i_2})\,
     T_2({\color{blue}i_2}, {\color{red}i_1})\,
     T_3({\color{red}i_1}, i_3)$\\[6pt]
  \begin{tikzpicture}
    \node[point, fill=blue!12] (v2) at (0, 0)   {\textcolor{blue}{$2$}};
    \node[point, fill=red!12]  (v1) at (1.2, 0) {\textcolor{red}{$1$}};
    \node[point]               (v3) at (2.4, 0) {$3$};
    \draw (v2) -- (v1) -- (v3);
  \end{tikzpicture}};

% ================= left branch: eliminate vertex 1 first =================
\node[state] (L) at (-4.2, -5.2) {%
  $\displaystyle \sum_{{\color{blue}i_2},\, i_3 \,\in\, [n]}
     T_4({\color{blue}i_2}, i_3)$\\[6pt]
  \begin{tikzpicture}
    \node[point, fill=blue!12] (v2) at (0, 0)   {\textcolor{blue}{$2$}};
    \node[point, draw=none]         at (1.2, 0) {\phantom{$1$}};
    \node[point]               (v3) at (2.4, 0) {$3$};
    \draw (v2) to[bend right=20] node[below, font=\scriptsize] {fill-in} (v3);
  \end{tikzpicture}};

% ================= right branch: eliminate vertex 2 first =================
\node[state] (R) at (4.2, -5.2) {%
  $\displaystyle \sum_{{\color{red}i_1},\, i_3 \,\in\, [n]}
     T_5({\color{red}i_1})\, T_3({\color{red}i_1}, i_3)$\\[6pt]
  \begin{tikzpicture}
    \node[point, draw=none]        at (0, 0)   {\phantom{$2$}};
    \node[point, fill=red!12] (v1) at (1.2, 0) {\textcolor{red}{$1$}};
    \node[point]              (v3) at (2.4, 0) {$3$};
    \draw (v1) -- (v3);
  \end{tikzpicture}};

% ================= the two alternative first steps =================
\draw[arrow] (root) -- node[midway, left, align=right, xshift=-4pt] {%
  sum over ${\color{red}i_1}$ $\Longleftrightarrow$ eliminate vertex ${\color{red}1}$,
  \quad $\degp{{\color{red}1}}{G_{\calA}} = 2$:\\[2pt]
  $T_4({\color{blue}i_2}, i_3) \coloneqq \sum_{{\color{red}i_1}}
     T_1({\color{red}i_1}, {\color{blue}i_2})\, T_2({\color{blue}i_2}, {\color{red}i_1})\, T_3({\color{red}i_1}, i_3)$\\[2pt]
  $\underbrace{n^{2}}_{\text{entries}} \times \underbrace{n}_{\text{adds}}
     \times \underbrace{3}_{\text{factors}} = \less(n^{3})$%
} (L);
\draw[arrow] (root) -- node[midway, right, align=left, xshift=4pt] {%
  sum over ${\color{blue}i_2}$ $\Longleftrightarrow$ eliminate vertex ${\color{blue}2}$,
  \quad $\degp{{\color{blue}2}}{G_{\calA}} = 1$:\\[2pt]
  $T_5({\color{red}i_1}) \coloneqq \sum_{{\color{blue}i_2}}
     T_1({\color{red}i_1}, {\color{blue}i_2})\, T_2({\color{blue}i_2}, {\color{red}i_1})$\\[2pt]
  $\underbrace{n}_{\text{entries}} \times \underbrace{n}_{\text{adds}}
     \times \underbrace{2}_{\text{factors}} = \less(n^{2})$%
} (R);

% ================= completions =================
\node[note, below=1.1cm of L] (Lend) {%
  then eliminate ${\color{blue}2}, 3$ at degrees $1, 0$\\[2pt]
  $\Longrightarrow$ total $\less(n^{3})$};
\node[note, below=1.1cm of R] (Rend) {%
  then eliminate ${\color{red}1}, 3$ at degrees $1, 0$\\[2pt]
  $\Longrightarrow$ total $\less(n^{2}) = \less\big(n^{\treewidthp{G_{\calA}} + 1}\big)$};
\draw[arrow, dashed] (L) -- (Lend);
\draw[arrow, dashed] (R) -- (Rend);
\end{tikzpicture}%
}
\caption{\rev{The correspondence between partial summation and vertex elimination, illustrated on the \Einsum{} notation $\calA = ((1,2), (2,1), (1,3))$, whose decomposition graph is that of Figure~\ref{fig:graph_hoif_4_2}. The two branches show the two possible first steps: summing over ${\color{red}i_1}$ (left) versus summing over ${\color{blue}i_2}$ (right), each transforming the expression in exact parallel with eliminating the corresponding vertex. Note the fill-in edge $2-3$ in the left branch, created because the intermediate tensor $T_4$ carries both neighbors of the eliminated vertex as its indices.}}
\label{fig:elimination_sketch}
\end{figure}

\rev{Two consequences of this vertex-elimination view deserve emphasis. The first concerns \emph{space}. When an index is summed out, the intermediate tensor that results is indexed exactly by the neighbours of that index in the current decomposition graph (Definition~\ref{def:vertex_elimination}); along an ordering that attains the treewidth, every stored intermediate therefore carries at most $\treewidthp{G_\calA}$ indices, i.e.\ at most $n^{\treewidthp{G_\calA}}$ entries. Hence the algorithm of Proposition~\ref{pro:complexity_einsum}---which sums over \emph{one index at a time}, a scheme we call \emph{index-wise}---evaluates $\tensorcontraction(\calT, \calA)$ in time $\less(|\calA|\, n^{\treewidthp{G_\calA}+1})$ and peak working memory $\less(n^{\treewidthp{G_\calA}})$ beyond the input tensors. Throughout the main text we report the time and space complexity of this index-wise scheme.}

\rev{The second concerns how the \Einsum{} operation is executed in practice. The contraction engines of \torcheinsum{} and \opteinsum{} do not sum over one index at a time; they contract \emph{two tensors at a time}, a scheme we call \emph{tensor-wise} (also called \emph{pairwise}), each step being a \emph{tensor contraction} (an \Einsum{} operation in its own right, possibly summing several indices) that is executed as a highly optimized, GPU-friendly matrix multiplication. The two schemes are alike in that both follow an elimination/contraction ordering and both attain the optimal time complexity $\less(n^{\treewidthp{G_\calA}+1})$, but they form different intermediate tensors and hence may differ in their memory footprint, and not every library exposes the full set of tensor-wise operations---\torcheinsum{} does, whereas \numpyeinsum{} does not. We make the tensor-wise scheme precise, prove that it attains the same time-complexity upper bound, and explain why the \Einsum{} libraries adopt it---while its space complexity calls for a more delicate discussion---in Appendix~\ref{app:index_tensor}.}

\rev{%
\begin{example}\label{eg:index_tensor}
Let $\calA_\diamond = ((1,2),(2,3),(3,4),(4,1),(1,3))$, with a single matrix $T$ shared by all five tuples, so that
\begin{align*}
\tensorcontraction(\calT, \calA_\diamond) = \sum_{i_1, i_2, i_3, i_4 \in [n]} T(i_1, i_2)\, T(i_2, i_3)\, T(i_3, i_4)\, T(i_4, i_1)\, T(i_1, i_3).
\end{align*}
The decomposition graph $G_{\calA_\diamond}$ has vertices $\{1,2,3,4\}$ and edges $\{1,2\}, \{2,3\}, \{3,4\}, \{1,4\}, \{1,3\}$---the complete graph $\completegraphp{4}$ with the single edge $\{2,4\}$ removed---and $\treewidthp{G_{\calA_\diamond}} = 2$: eliminating first $2$ and then $4$, each has degree $2$ with already-adjacent neighbours $1$ and $3$, so no fill-in edge appears, after which $1$ and $3$ are eliminated at degrees $1$ and $0$ (Figure~\ref{fig:elim_diamond}). Table~\ref{tab:index_tensor} carries out both schemes (the index-wise one with the ordering $\sigma = (2,4,1,3)$): each row performs one operation, and the third column shows the sum that then remains for $V$. Both reach the scalar in four steps, paying the dominant cost when eliminating $2$ and $4$ and storing only $n \times n$ intermediates, so each runs in time $\less(n^3) = \less(n^{\treewidthp{G_{\calA_\diamond}}+1})$ and space $\less(n^2) = \less(n^{\treewidthp{G_{\calA_\diamond}}})$. They differ only in how the tail is organized: after $2$ and $4$ are eliminated, the matrices $s_2, s_4$ (resp.\ $U_1, U_2$) and $T$ all carry the indices $\{1,3\}$, and the index-wise scheme sums $i_1$ over all three in its single step~3, whereas the tensor-wise scheme, contracting two tensors at a time, first multiplies $U_1$ and $U_2$ (step~3) and only then contracts with $T$ (step~4). We define the tensor-wise scheme precisely in Appendix~\ref{app:index_tensor}.
\end{example}%
}

\begin{table}[htbp]
\centering
\caption{\rev{The two schemes computing $\tensorcontraction(\calT, \calA_\diamond)$ of Example~\ref{eg:index_tensor}. Each row performs one operation, and the ``remaining sum for $V$'' column shows what is then left to compute; $s_2, s_4, U_1, U_2, W$ are $n \times n$ arrays and $s_1$ a length-$n$ vector. The \emph{time} column counts the multiplications per summand ($1$ for a two-factor product, $2$ for three; the last index-wise step adds $n$ scalars); the \emph{space} column is the total size of the tensors in memory after the step, including the input $T$ until its last use---hence $3n^2$ at step~2, where $T$ and the two $n \times n$ tensors ($s_2, s_4$ resp.\ $U_1, U_2$) coexist. Both schemes run in $\appequal(n^3)$ time and peak working memory $3n^2 = \appequal(n^{\treewidthp{G_{\calA_\diamond}}})$.}}
\label{tab:index_tensor}
\rev{\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c l l c c}
\toprule
\multicolumn{5}{c}{\textbf{Index-wise} (one index at a time)} \\
\cmidrule(lr){1-5}
 & operation & remaining sum for $V$ & time & space \\
\midrule
 &  & $\sum_{i_1,i_2,i_3,i_4} T(i_1,i_2)\, T(i_2,i_3)\, T(i_3,i_4)\, T(i_4,i_1)\, T(i_1,i_3)$ &  & $n^2$ \\
\midrule
1 & $s_2(i_1,i_3) = \sum_{i_2} T(i_1,i_2)\, T(i_2,i_3)$ & $\sum_{i_1,i_3,i_4} s_2(i_1,i_3)\, T(i_3,i_4)\, T(i_4,i_1)\, T(i_1,i_3)$ & $n^3$ & $2n^2$ \\
2 & $s_4(i_1,i_3) = \sum_{i_4} T(i_3,i_4)\, T(i_4,i_1)$ & $\sum_{i_1,i_3} s_2(i_1,i_3)\, s_4(i_1,i_3)\, T(i_1,i_3)$ & $n^3$ & $3n^2$ \\
3 & $s_1(i_3) = \sum_{i_1} s_2(i_1,i_3)\, s_4(i_1,i_3)\, T(i_1,i_3)$ & $\sum_{i_3} s_1(i_3)$ & $2n^2$ & $n$ \\
4 & $V = \sum_{i_3} s_1(i_3)$ & $V$ & $n$ & $1$ \\
\bottomrule
\end{tabular}
...
\begin{tabular}{c l l c c}
\toprule
\multicolumn{5}{c}{\textbf{Tensor-wise} (two tensors at a time)} \\
\cmidrule(lr){1-5}
 & operation & remaining sum for $V$ & time & space \\
\midrule
 &  & $\sum_{i_1,i_2,i_3,i_4} T(i_1,i_2)\, T(i_2,i_3)\, T(i_3,i_4)\, T(i_4,i_1)\, T(i_1,i_3)$ &  & $n^2$ \\
\midrule
1 & $U_1(i_1,i_3) = \sum_{i_2} T(i_1,i_2)\, T(i_2,i_3)$ & $\sum_{i_1,i_3,i_4} U_1(i_1,i_3)\, T(i_3,i_4)\, T(i_4,i_1)\, T(i_1,i_3)$ & $n^3$ & $2n^2$ \\
2 & $U_2(i_1,i_3) = \sum_{i_4} T(i_3,i_4)\, T(i_4,i_1)$ & $\sum_{i_1,i_3} U_1(i_1,i_3)\, U_2(i_1,i_3)\, T(i_1,i_3)$ & $n^3$ & $3n^2$ \\
3 & $W(i_1,i_3) = U_1(i_1,i_3)\, U_2(i_1,i_3)$ & $\sum_{i_1,i_3} W(i_1,i_3)\, T(i_1,i_3)$ & $n^2$ & $2n^2$ \\
4 & $V = \sum_{i_1,i_3} W(i_1,i_3)\, T(i_1,i_3)$ & $V$ & $n^2$ & $1$ \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{tikzpicture}[
  every node/.style={font=\small},
  vtx/.style={draw, circle, inner sep=1pt, minimum size=0.5cm},
  arr/.style={-{Latex}, thick}]
% G_0 : K_4 minus edge {2,4}  (two triangles 1-2-3 and 1-3-4 sharing edge 1-3)
\node[vtx] (a1) at (-0.8,0)  {$1$};
\node[vtx] (a3) at (0.8,0)   {$3$};
\node[vtx] (a2) at (0,0.85)  {$2$};
\node[vtx] (a4) at (0,-0.85) {$4$};
\draw (a1)--(a2) (a2)--(a3) (a1)--(a4) (a4)--(a3) (a1)--(a3);
% G_1 : triangle on {1,3,4}
\node[vtx] (b1) at (3.2,0)   {$1$};
\node[vtx] (b3) at (4.8,0)   {$3$};
\node[vtx] (b4) at (4,-0.85) {$4$};
\draw (b1)--(b3) (b1)--(b4) (b3)--(b4);
% G_2 : edge {1,3}
\node[vtx] (c1) at (7.1,0) {$1$};
\node[vtx] (c3) at (7.9,0) {$3$};
\draw (c1)--(c3);
% G_3 : single vertex 3
\node[vtx] (d3) at (10,0) {$3$};
% end
\node (e) at (11.9,0) {$\varnothing$};
% elimination arrows, labelled with the degree of the eliminated vertex
\draw[arr] (1.15,0) -- (3.0,0)  node[midway, above, align=center, font=\scriptsize] {elim.~$2$\\ $\deg 2$};
\draw[arr] (5.05,0) -- (6.9,0)  node[midway, above, align=center, font=\scriptsize] {elim.~$4$\\ $\deg 2$};
\draw[arr] (8.15,0) -- (9.7,0)  node[midway, above, align=center, font=\scriptsize] {elim.~$1$\\ $\deg 1$};
\draw[arr] (10.45,0) -- (11.5,0) node[midway, above, align=center, font=\scriptsize] {elim.~$3$\\ $\deg 0$};
\end{tikzpicture}
\caption{\rev{Vertex elimination of the decomposition graph $G_{\calA_\diamond}$ of Example~\ref{eg:index_tensor} under the ordering $\sigma = (2,4,1,3)$; each arrow eliminates one vertex and is labelled with its degree at that step. No fill-in edge is ever created (the neighbours $1,3$ of the eliminated $2$ and $4$ are already adjacent), and the largest degree along the elimination is $2$, so $\treewidthp{G_{\calA_\diamond}} = 2$ and $\tensorcontraction(\calT, \calA_\diamond)$ costs $\less(n^{\treewidthp{G_{\calA_\diamond}}+1}) = \less(n^3)$.}}
\label{fig:elim_diamond}
\end{figure}
...
We take matrix multiplication as an example to illustrate the \Einsum{} operation just defined. Let $T_1, T_2$ be two input matrices, which are also 2nd-order tensors. Therefore $\calT = (T_1, T_2)$. Suppose we are interested in multiplying $T_1$ and $T_2$, denoted as $T^{\rm out}$: for any $i, j \in [n]$, 
\rev{ 
\begin{align*} 
   T^{\rm out} (i, j) = \sum\limits_{k = 1}^n T_{1} (i, k) T_{2} (k, j).
\end{align*}
 }
...
\section{Index-wise versus Tensor-wise Execution of the \Einsum{} Operation}
\label{app:index_tensor}

\rev{The proof of Proposition~\ref{pro:complexity_einsum} in Appendix~\ref{app:complexity_einsum} evaluates an \Einsum{} operation by summing over \emph{one index at a time}: each step of the \Einsum{} notation sequence (Definition~\ref{def:einsum_sequence}) eliminates a single index and, by Lemma~\ref{lem:einsum_graph_seq}, a single vertex of the decomposition graph. We call this the \emph{index-wise} scheme. The contraction engines of \torcheinsum{} and \opteinsum{}, by contrast, evaluate a contraction by repeatedly combining \emph{two tensors at a time}; we call this the \emph{tensor-wise} scheme (it is also called \emph{pairwise}). The two differ in the intermediate quantities they form: the index-wise scheme keeps a single running list of tensors and peels off one index per step, whereas the tensor-wise scheme builds a binary tree of pairwise tensor contractions, each executed as a matrix multiplication. In this appendix we make the tensor-wise scheme precise, prove that its optimal time complexity matches the index-wise bound of Proposition~\ref{pro:complexity_einsum}, namely $\less(|\calA|\, n^{\treewidthp{G_\calA}+1})$, and explain why the \Einsum{} libraries adopt it (Remark~\ref{rem:why_tensorwise}). The space complexity is more subtle, and we discuss it at the end.}

\rev{Throughout, $\calA = (A_1, \dots, A_K)$ is an \Einsum{} notation with no output and index set $[m]$, and $\calT = (T_1, \dots, T_K)$ is the corresponding tuple of tensors of size $n$; the goal is the scalar $\tensorcontraction(\calT, \calA) = \sum_{\alpha \in [n]^m} \prod_{k=1}^K T_k(\alpha[A_k])$. As before we identify each signature with its index set $\takesetp{A_k}$. We recall from Definition~\ref{def:treewidth_elimination} the elimination sequence $G^\sigma_0 = G_\calA$, $G^\sigma_i = \eliminatevg{\sigma[i]}{G^\sigma_{i-1}}$ of a permutation $\sigma$ of $[m]$, and its elimination width $\mathsf{w}(\sigma) \coloneqq \max_{i \in [m]} \degp{\sigma[i]}{G^\sigma_{i-1}}$, so that $\treewidthp{G_\calA} = \min_\sigma \mathsf{w}(\sigma)$.}

\subsection{The tensor-wise computation model}

\rev{We first abstract the two operations from which \torcheinsum{} and \opteinsum{} build a contraction: a binary contraction of two tensors, and a unary reduction of one tensor.}

\begin{definition}[Tensor-wise primitives]
\label{def:tw_primitives}
\rev{A \emph{primitive} acts on the current list of tensors, consumes its operand(s), and appends one new tensor.
\begin{itemize}
\item[\textnormal{(P1)}] \emph{Binary contraction.} Choose two distinct tensors $T_a, T_b$ from the current list, with signatures $B_a, B_b$, and a (possibly empty) set $C \subseteq \takesetp{B_a} \cup \takesetp{B_b}$ such that no index of $C$ occurs in the signature of any tensor other than $T_a$ and $T_b$. The output has signature $(\takesetp{B_a} \cup \takesetp{B_b}) \setminus C$ and entries
\begin{align*}
T_c(\gamma) = \sum_{\alpha \in [n]^{C}} T_a\big((\gamma, \alpha)[B_a]\big)\, T_b\big((\gamma, \alpha)[B_b]\big),
\end{align*}
with time cost $\less\!\big(n^{|\takesetp{B_a} \cup \takesetp{B_b}|}\big)$ and output size $n^{|(\takesetp{B_a} \cup \takesetp{B_b}) \setminus C|}$. The case $C = \emptyset$ is an entrywise (broadcast) product, and if moreover $\takesetp{B_a} \cap \takesetp{B_b} = \emptyset$ it is an outer product.
\item[\textnormal{(P2)}] \emph{Unary reduction.} Choose one tensor $T_a$ with signature $B_a$ and a nonempty $C \subseteq \takesetp{B_a}$ such that no index of $C$ occurs in any other signature. The output has signature $\takesetp{B_a} \setminus C$, entries $T_c(\gamma) = \sum_{\alpha \in [n]^C} T_a\big((\gamma, \alpha)[B_a]\big)$, time cost $\less(n^{|B_a|})$, and output size $n^{|B_a| - |C|}$.
\end{itemize}
The requirement that an eliminated index $i \in C$ occur only in the operands is a \emph{legality} condition: $i$ may be summed away only once every tensor carrying it has been gathered into the present operation.}
\end{definition}

\begin{definition}[Tensor-wise schedule and induced ordering]
\label{def:tw_schedule}
\rev{A \emph{tensor-wise schedule} $\Sigma$ for $(\calT, \calA)$ is a finite sequence of primitives that, started from $\calT$, terminates with a single scalar. Its time cost $\mathsf{time}(\Sigma)$ is the sum of the time costs of its primitives; its \emph{auxiliary space} $\mathsf{aux}(\Sigma)$ is the maximum, over the execution, of the total number of entries of all currently allocated non-input tensors (each tensor freed once it is last used; the inputs, of total size $\sum_k n^{|A_k|}$, are counted separately). Each index of $[m]$ is eliminated through the set $C$ of exactly one primitive, so $\Sigma$ determines a permutation $\sigma$ of $[m]$ ordering the indices by elimination (indices eliminated by the same primitive may be ordered arbitrarily among themselves); any such $\sigma$ is said to be \emph{induced} by $\Sigma$, written $\sigma \vartriangleleft \Sigma$.}
\end{definition}

\subsection{Tensor-wise execution attains the index-wise time bound}

\rev{The crux is that any elimination ordering can be realized by a tensor-wise schedule of the same time order.}

\begin{lemma}[Simulating an elimination ordering]
\label{lem:tw_upper}
\rev{For every permutation $\sigma$ of $[m]$ there is a tensor-wise schedule $\Sigma_\sigma$ with $\sigma \vartriangleleft \Sigma_\sigma$ and
\begin{align*}
\mathsf{time}(\Sigma_\sigma) \le (K + m)\, n^{1 + \mathsf{w}(\sigma)} + K, \qquad \mathsf{aux}(\Sigma_\sigma) \le 2\, n^{1 + \mathsf{w}(\sigma)} + m\, n^{\mathsf{w}(\sigma)},
\end{align*}
the latter being at most $3\, n^{1 + \mathsf{w}(\sigma)}$ when $m \le n$.}
\end{lemma}

\begin{proof}
\rev{We process $i = 1, \dots, m$, maintaining the invariant that after step $i$ the current list of signatures consists of the tuples of $\calA^\sigma_i$ (Definition~\ref{def:einsum_sequence}), together with possibly some scalars. This holds at the start with $\calA^\sigma_0 = \calA$. At step $i$, let $T_{(1)}, \dots, T_{(r)}$ be the current tensors whose signature contains $\sigma[i]$, with $r \ge 1$ by the invariant. If $r = 1$, apply (P2) to $T_{(1)}$ with $C = \{\sigma[i]\}$, which is legal because $\sigma[i]$ occurs in no other signature. If $r \ge 2$, form the running products $W_1 \coloneqq T_{(1)}$ and $W_j \coloneqq \textnormal{(P1)}(W_{j-1}, T_{(j)}, C = \emptyset)$ for $j = 2, \dots, r$, then apply (P2) to $W_r$ with $C = \{\sigma[i]\}$, now legal because every tensor that contained $\sigma[i]$ has been absorbed into $W_r$. In both cases the step replaces the tuples containing $\sigma[i]$ by the single tuple $\bigcup_{j \le r} \takesetp{B_{(j)}} \setminus \{\sigma[i]\}$ (or a scalar), exactly the replacement rule defining $\calA^\sigma_i$; the invariant is preserved, and only $\sigma[i]$ is eliminated at step $i$, so $\sigma \vartriangleleft \Sigma_\sigma$. After step $m$ at most $K$ scalars remain, which are multiplied pairwise into one.

\emph{Time.} Write $d_i \coloneqq \degp{\sigma[i]}{G^\sigma_{i-1}}$. By Lemma~\ref{lem:einsum_graph_seq}, every tuple of $\calA^\sigma_{i-1}$ containing $\sigma[i]$ is a subset of $\{\sigma[i]\} \cup \neighborvg{\sigma[i]}{G^\sigma_{i-1}}$, so every running signature obeys $\takesetp{W_j} \subseteq \{\sigma[i]\} \cup \neighborvg{\sigma[i]}{G^\sigma_{i-1}}$ and hence $|\takesetp{W_j}| \le d_i + 1 \le \mathsf{w}(\sigma) + 1$. Each primitive of step $i$ therefore costs at most $\less(n^{1 + \mathsf{w}(\sigma)})$. Each step consumes $r_i$ tensors and produces one, so $\sum_i (r_i - 1) = K - c$ with $c \ge 1$ the number of leftover scalars; thus there are $\sum_i r_i \le K + m$ such primitives, plus $c - 1 \le K$ final scalar multiplications of cost $1$. Summing gives the stated time bound.

\emph{Space.} At any instant of step $i$ the allocated non-input tensors are: at most two running tensors $W_{j-1}, W_j$, each of at most $n^{1 + \mathsf{w}(\sigma)}$ entries; and the outputs of earlier steps not yet consumed, each of signature size $\degp{\sigma[j]}{G^\sigma_{j-1}} \le \mathsf{w}(\sigma)$, hence at most $n^{\mathsf{w}(\sigma)}$ entries and fewer than $m$ in number. Inputs are freed when consumed. The auxiliary-space bound follows.}
\end{proof}

\rev{For the matching lower bound we use one bookkeeping fact about eliminating a \emph{set} of vertices, and one invariant. For $S \subseteq \verticesp{G}$, write $G \ominus S$ for the graph obtained by eliminating the vertices of $S$ one by one.}

\begin{lemma}[Set elimination]
\label{lem:set_elim}
\rev{The graph $G \ominus S$ does not depend on the order in which the vertices of $S$ are eliminated, and two vertices $u, v \notin S$ are adjacent in it if and only if $G$ contains a $u$--$v$ path all of whose internal vertices lie in $S$ (a direct edge counting as such a path). In particular, with $E_{i-1} \coloneqq \{\sigma[1], \dots, \sigma[i-1]\}$ one has $G^\sigma_{i-1} = G_\calA \ominus E_{i-1}$.}
\end{lemma}

\begin{proof}
\rev{Induction on $|S|$; the case $S = \emptyset$ is trivial. Eliminate $s \in S$ first, setting $G' \coloneqq \eliminatevg{s}{G}$: then $u, v$ are adjacent in $G'$ iff they are adjacent in $G$ or are both neighbours of $s$, i.e.\ iff $G$ has a $u$--$v$ path internal to $\{s\}$. Applying the inductive hypothesis to $(G', S \setminus \{s\})$ and expanding each $G'$-edge of a witnessing path either as a $G$-edge or as a length-two path through $s$ (and conversely, replacing a sub-walk $x$--$s$--$y$ of a $G$-path by the $G'$-edge $x$--$y$) gives the claim, which is therefore order-independent.}
\end{proof}

\rev{Call two surviving indices \emph{co-resident} at time $t$ (after $t$ primitives) if some current signature contains both, and let $D_t$ be the set of indices eliminated by the first $t$ primitives, so $H_t \coloneqq G_\calA \ominus D_t$ is well defined.}

\begin{lemma}[Co-residence invariant]
\label{lem:tw_coresidence}
\rev{For every schedule $\Sigma$ and every $t \ge 0$, every edge of $H_t$ joins two co-resident indices.}
\end{lemma}

\begin{proof}
\rev{Induction on $t$. At $t = 0$, the edges of $H_0 = G_\calA$ are exactly the co-occurrences within the input signatures. For the step, let the $(t{+}1)$-st primitive have operand index union $\mathcal{O}$ (one or two signatures) and elimination set $C$, so $D_{t+1} = D_t \cup C$ and the output signature is $\mathcal{O} \setminus C$; by legality, every time-$t$ signature meeting $C$ is an operand. Let $\{u, v\}$ be an edge of $H_{t+1}$. By Lemma~\ref{lem:set_elim} there is a $u$--$v$ path in $G_\calA$ internal to $D_t \cup C$; contracting its maximal runs of $D_t$-vertices (each joining two $H_t$-adjacent vertices) gives an $H_t$-path $u - z_1 - \cdots - z_q - v$ with $z_1, \dots, z_q \in C$. If $q = 0$, then $\{u, v\}$ is an $H_t$-edge; its witness tensor either survives (keeping $u, v$ co-resident) or is an operand, in which case $u, v \in \mathcal{O}$ and, as $u, v \notin C$, they lie in the output signature. If $q \ge 1$, the witness of $\{u, z_1\}$ contains $z_1 \in C$, hence is an operand, so $u \in \mathcal{O}$; symmetrically $v \in \mathcal{O}$; as $u, v \notin C$ they lie in the output signature. In all cases $u, v$ are co-resident at time $t + 1$.}
\end{proof}

\begin{lemma}[Time lower bound]
\label{lem:tw_lower}
\rev{For every tensor-wise schedule $\Sigma$ and every $\sigma \vartriangleleft \Sigma$, $\mathsf{time}(\Sigma) \ge n^{1 + \mathsf{w}(\sigma)} \ge n^{\treewidthp{G_\calA}+1}$.}
\end{lemma}

\begin{proof}
\rev{Fix $i$ and let the primitive eliminating $c \coloneqq \sigma[i]$ be the $(t{+}1)$-st, with operand index union $\mathcal{O}$ and $c \in C$; then $E_{i-1} = D_t \cup C_{<}$ for some $C_{<} \subseteq C$. For $u \in \neighborvg{c}{G^\sigma_{i-1}}$, Lemma~\ref{lem:set_elim} gives a $c$--$u$ path in $G_\calA$ internal to $E_{i-1} \subseteq D_t \cup C$; contracting its $D_t$-runs yields an $H_t$-path $c - y_1 - \cdots - y_s - u$ with $y_j \in C$. Walking along it and using the co-residence invariant (Lemma~\ref{lem:tw_coresidence}), each witness tensor contains an index of $C$, hence is an operand, forcing $y_1, \dots, y_s, u \in \mathcal{O}$; also $c \in \mathcal{O}$. Thus $\mathcal{O} \supseteq \{c\} \cup \neighborvg{c}{G^\sigma_{i-1}}$ and the primitive costs at least $n^{1 + \degp{c}{G^\sigma_{i-1}}}$. Taking the most expensive step and minimizing over orderings, $\mathsf{time}(\Sigma) \ge n^{1 + \mathsf{w}(\sigma)} \ge n^{\treewidthp{G_\calA}+1}$.}
\end{proof}

\begin{proposition}[Tensor-wise time equals index-wise time]
\label{pro:tw_time}
\rev{Let $C^\ast(\calA)$ be the minimum of $\mathsf{time}(\Sigma)$ over all tensor-wise schedules. Then
\begin{align*}
n^{\treewidthp{G_\calA}+1} \;\le\; C^\ast(\calA) \;\le\; (K + m)\, n^{\treewidthp{G_\calA}+1} + K,
\end{align*}
so that, for fixed \Einsum{} structure $\calA$ and $n \to \infty$, $C^\ast(\calA) = \appequal(n^{\treewidthp{G_\calA}+1})$. This is the same time-complexity upper bound, $\less(|\calA|\, n^{\treewidthp{G_\calA}+1})$, as the index-wise algorithm of Proposition~\ref{pro:complexity_einsum}.}
\end{proposition}

\begin{proof}
\rev{The upper bound applies Lemma~\ref{lem:tw_upper} to a permutation $\sigma^\ast$ with $\mathsf{w}(\sigma^\ast) = \treewidthp{G_\calA}$, which exists by the elimination characterization of treewidth, using $K = |\calA|$ and $m \le \treewidthp{G_\calA}+1$ being dominated by $n^{\treewidthp{G_\calA}+1}$. The lower bound is Lemma~\ref{lem:tw_lower}. As with Proposition~\ref{pro:complexity_einsum}, finding such a $\sigma^\ast$ is NP-hard in general \citep{arnborg1987complexity}; the statement concerns the cost of \emph{executing} an optimal schedule, not of \emph{finding} one.}
\end{proof}

\subsection{Space complexity}

\rev{The space behaviour of the two schemes is less clean than their time behaviour. As noted after Proposition~\ref{pro:complexity_einsum}, the index-wise scheme stores intermediates of at most $\treewidthp{G_\calA}$ indices, so its peak working memory is $\less(n^{\treewidthp{G_\calA}})$. The construction of Lemma~\ref{lem:tw_upper} shows that the tensor-wise scheme can be run in auxiliary space $\less(n^{\treewidthp{G_\calA}+1})$ (when $m \le n$). This bound is the best possible in general: when an index occurs in three or more tensors whose signatures already cover all of its current neighbours, no pairwise schedule can avoid materializing an intermediate of $\treewidthp{G_\calA}+1$ indices.\footnote{\rev{The minimal largest tensor-wise intermediate is, precisely, the carving width of the contraction, a graph parameter lying between $\treewidthp{G_\calA}$ and $\treewidthp{G_\calA}+1$; see \citet{markov2008simulating, gray2021hyper} for the tensor-network theory behind it. We do not pursue this characterization here.}} Often, however, the tensor-wise scheme still matches the index-wise $\less(n^{\treewidthp{G_\calA}})$: this happens whenever no intermediate carries more than $\treewidthp{G_\calA}$ indices, as in Example~\ref{eg:index_tensor} below.}

\rev{The tensor-wise scheme of Example~\ref{eg:index_tensor} (Table~\ref{tab:index_tensor}) illustrates this: its intermediates $U_1, U_2, W$ are all $n \times n$ and its final contraction sums $\{1,3\}$ over $W$ and $T$, so no tensor exceeds $n \times n$ and the peak auxiliary space is $\less(n^2) = \less(n^{\treewidthp{G_{\calA_\diamond}}})$, matching the index-wise scheme.}

\rev{The next two examples sharpen the contrast between this matching case and the worst case. Both have the \emph{same} decomposition graph $\completegraphp{4}$, so $\treewidthp{G_\calA} = 3$ and the index-wise scheme uses peak memory $n^3$ in each; yet their tensor-wise schemes diverge.}

\begin{example}[every index in two tensors]
\label{eg:space_K4}
\rev{Let $\calA_1 = ((1,2),(1,3),(1,4),(2,3),(2,4),(3,4))$, the six edges of $\completegraphp{4}$, all sharing one matrix $T$. The index-wise scheme eliminating $1$ first forms $s_1(i_2,i_3,i_4) = \sum_{i_1} T(i_1,i_2)\, T(i_1,i_3)\, T(i_1,i_4)$, an $n^3$ tensor, after which $2, 3, 4$ are removed from smaller tensors; its peak is $n^3$ (Table~\ref{tab:space_K4}, top). The tensor-wise scheme (Table~\ref{tab:space_K4}, bottom) also stays at $n^3$: it merges two of the three tensors carrying $1$ into the $n^3$ tensor $W_1$ on $\{1,2,3\}$, then contracts $W_1$ with $T(1,4)$ while summing $i_1$---a single binary contraction whose output is the $n^3$ tensor on $\{2,3,4\}$---so the order-four tensor on $\{1,2,3,4\}$ is never formed. Both schemes use peak memory $n^3 = n^{\treewidthp{G_{\calA_1}}}$.}
\end{example}

\begin{sidewaystableorg}
\centering
\caption{\rev{Both schemes computing $\tensorcontraction(\calT, \calA_1)$ of Example~\ref{eg:space_K4} (the edges of $\completegraphp{4}$), in the format of Table~\ref{tab:index_tensor}; the space column counts the input matrix $T$ ($n^2$) until its last use. No intermediate exceeds $n^3$, so both schemes use peak memory $\appequal(n^3) = \appequal(n^{\treewidthp{G_{\calA_1}}})$ and run in $\appequal(n^4) = \appequal(n^{\treewidthp{G_{\calA_1}}+1})$ time.}}
\label{tab:space_K4}
\rev{\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c l l c c}
...
\cmidrule(lr){1-5}
 & operation & remaining sum for $V$ & time & space \\
\midrule
 &  & $\sum_{i_1,i_2,i_3,i_4} T(i_1,i_2)\, T(i_1,i_3)\, T(i_1,i_4)\, T(i_2,i_3)\, T(i_2,i_4)\, T(i_3,i_4)$ &  & $n^2$ \\
\midrule
1 & $s_1(i_2,i_3,i_4) = \sum_{i_1} T(i_1,i_2)\, T(i_1,i_3)\, T(i_1,i_4)$ & $\sum_{i_2,i_3,i_4} s_1(i_2,i_3,i_4)\, T(i_2,i_3)\, T(i_2,i_4)\, T(i_3,i_4)$ & $2n^4$ & $n^3 + n^2$ \\
2 & $s_2(i_3,i_4) = \sum_{i_2} s_1(i_2,i_3,i_4)\, T(i_2,i_3)\, T(i_2,i_4)$ & $\sum_{i_3,i_4} s_2(i_3,i_4)\, T(i_3,i_4)$ & $2n^3$ & $2n^2$ \\
3 & $s_3(i_4) = \sum_{i_3} s_2(i_3,i_4)\, T(i_3,i_4)$ & $\sum_{i_4} s_3(i_4)$ & $n^2$ & $n$ \\
4 & $V = \sum_{i_4} s_3(i_4)$ & $V$ & $n$ & $1$ \\
\midrule
\multicolumn{5}{c}{\textbf{Tensor-wise} (two tensors at a time)} \\
\cmidrule(lr){1-5}
 & operation & remaining sum for $V$ & time & space \\
\midrule
 &  & $\sum_{i_1,i_2,i_3,i_4} T(i_1,i_2)\, T(i_1,i_3)\, T(i_1,i_4)\, T(i_2,i_3)\, T(i_2,i_4)\, T(i_3,i_4)$ &  & $n^2$ \\
\midrule
1 & $W_1(i_1,i_2,i_3) = T(i_1,i_2)\, T(i_1,i_3)$ & $\sum_{i_1,i_2,i_3,i_4} W_1(i_1,i_2,i_3)\, T(i_1,i_4)\, T(i_2,i_3)\, T(i_2,i_4)\, T(i_3,i_4)$ & $n^3$ & $n^3 + n^2$ \\
2 & $U_1(i_2,i_3,i_4) = \sum_{i_1} W_1(i_1,i_2,i_3)\, T(i_1,i_4)$ & $\sum_{i_2,i_3,i_4} U_1(i_2,i_3,i_4)\, T(i_2,i_3)\, T(i_2,i_4)\, T(i_3,i_4)$ & $n^4$ & $n^3 + n^2$ \\
3 & $W_2(i_2,i_3,i_4) = U_1(i_2,i_3,i_4)\, T(i_2,i_3)$ & $\sum_{i_2,i_3,i_4} W_2(i_2,i_3,i_4)\, T(i_2,i_4)\, T(i_3,i_4)$ & $n^3$ & $n^3 + n^2$ \\
4 & $U_2(i_3,i_4) = \sum_{i_2} W_2(i_2,i_3,i_4)\, T(i_2,i_4)$ & $\sum_{i_3,i_4} U_2(i_3,i_4)\, T(i_3,i_4)$ & $n^3$ & $2n^2$ \\
5 & $V = \sum_{i_3,i_4} U_2(i_3,i_4)\, T(i_3,i_4)$ & $V$ & $n^2$ & $1$ \\
\bottomrule
\end{tabular}
}
\end{sidewaystableorg}

\begin{example}[every index in three tensors]
\label{eg:space_triples}
\rev{Let $\calA_2 = ((1,2,3),(1,2,4),(1,3,4),(2,3,4))$, the four index-triples of $\{1,2,3,4\}$, all sharing one third-order tensor $T$; again $G_{\calA_2} = \completegraphp{4}$ and $\treewidthp{G_{\calA_2}} = 3$. The index-wise scheme eliminating the four indices in turn keeps every intermediate at most $n^3$ (Table~\ref{tab:space_triples}, top): the first forms $s_1(i_2,i_3,i_4) = \sum_{i_1} T(i_1,i_2,i_3)\, T(i_1,i_2,i_4)\, T(i_1,i_3,i_4)$, an $n^3$ tensor, after which the rest shrink. The tensor-wise scheme cannot match this (Table~\ref{tab:space_triples}, bottom). Every index now lies in three tensors, so no index can be summed by a first binary contraction; the first step must broadcast two of the four triples, and any two of them have index-union $\{1,2,3,4\}$, forcing an order-four tensor of size $n^4$. Hence the tensor-wise peak is $n^4 = n^{\treewidthp{G_{\calA_2}}+1}$, strictly larger than the index-wise $n^3$.}
\end{example}

\begin{sidewaystableorg}
\centering
\caption{\rev{Both schemes computing $\tensorcontraction(\calT, \calA_2)$ of Example~\ref{eg:space_triples} (the four triples), in the format of Table~\ref{tab:index_tensor}; the space column counts the input tensor $T$ ($n^3$) until its last use. The index-wise intermediates stay at $\appequal(n^3) = \appequal(n^{\treewidthp{G_{\calA_2}}})$, whereas the first tensor-wise step already forms the $n^4 = n^{\treewidthp{G_{\calA_2}}+1}$ tensor $W$ (any two of the four triples have index-union $\{1,2,3,4\}$). Both schemes run in $\appequal(n^4)$ time.}}
\label{tab:space_triples}
\rev{\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c l l c c}
\toprule
...
\cmidrule(lr){1-5}
 & operation & remaining sum for $V$ & time & space \\
\midrule
 &  & $\sum_{i_1,i_2,i_3,i_4} T(i_1,i_2,i_3)\, T(i_1,i_2,i_4)\, T(i_1,i_3,i_4)\, T(i_2,i_3,i_4)$ &  & $n^3$ \\
\midrule
1 & $s_1(i_2,i_3,i_4) = \sum_{i_1} T(i_1,i_2,i_3)\, T(i_1,i_2,i_4)\, T(i_1,i_3,i_4)$ & $\sum_{i_2,i_3,i_4} s_1(i_2,i_3,i_4)\, T(i_2,i_3,i_4)$ & $2n^4$ & $2n^3$ \\
2 & $s_2(i_3,i_4) = \sum_{i_2} s_1(i_2,i_3,i_4)\, T(i_2,i_3,i_4)$ & $\sum_{i_3,i_4} s_2(i_3,i_4)$ & $n^3$ & $n^2$ \\
3 & $s_3(i_4) = \sum_{i_3} s_2(i_3,i_4)$ & $\sum_{i_4} s_3(i_4)$ & $n^2$ & $n$ \\
4 & $V = \sum_{i_4} s_3(i_4)$ & $V$ & $n$ & $1$ \\
\midrule
\multicolumn{5}{c}{\textbf{Tensor-wise} (two tensors at a time)} \\
\cmidrule(lr){1-5}
...
\midrule
 &  & $\sum_{i_1,i_2,i_3,i_4} T(i_1,i_2,i_3)\, T(i_1,i_2,i_4)\, T(i_1,i_3,i_4)\, T(i_2,i_3,i_4)$ &  & $n^3$ \\
\midrule
1 & $W(i_1,i_2,i_3,i_4) = T(i_1,i_2,i_3)\, T(i_1,i_2,i_4)$ & $\sum_{i_1,i_2,i_3,i_4} W(i_1,i_2,i_3,i_4)\, T(i_1,i_3,i_4)\, T(i_2,i_3,i_4)$ & $n^4$ & $n^4 + n^3$ \\
2 & $U_1(i_2,i_3,i_4) = \sum_{i_1} W(i_1,i_2,i_3,i_4)\, T(i_1,i_3,i_4)$ & $\sum_{i_2,i_3,i_4} U_1(i_2,i_3,i_4)\, T(i_2,i_3,i_4)$ & $n^4$ & $2n^3$ \\
3 & $V = \sum_{i_2,i_3,i_4} U_1(i_2,i_3,i_4)\, T(i_2,i_3,i_4)$ & $V$ & $n^3$ & $1$ \\
\bottomrule
\end{tabular}
}
\end{sidewaystableorg}

\rev{The two examples share the same decomposition graph and the same index-wise space $n^3$, but their tensor-wise space is $n^3$ versus $n^4$. The gap is governed by how many tensors share each index---the hyperedge structure---rather than by the decomposition graph alone, which is exactly the carving-width phenomenon noted in the footnote above.}

\rev{The unary primitive (P2) is, moreover, genuinely needed for the two schemes to agree, and it is the point on which the libraries differ. Consider the distance-covariance signature $\calA = ((1,2),(3,4))$ (Figure~\ref{fig:graph_dcov_1}), whose decomposition graph is two disjoint edges with $\treewidthp{G_\calA}+1 = 2$. Reducing each $T_k$ to a scalar by (P2) costs $\less(n^2)$, and multiplying the two scalars then matches the bound of Proposition~\ref{pro:tw_time} in $\less(1)$ auxiliary space. If (P2) is unavailable---so that the only admissible binary contractions sum indices shared by \emph{both} operands, as in a \texttt{tensordot} (matrix-multiplication) call---then on the disjoint signatures $\{1,2\}$ and $\{3,4\}$ the first step is forced to be an outer product, of cost and stored size $n^4$. This is precisely the gap seen between the two libraries on the distance-covariance computation of Section~\ref{sec:dependence}: \torcheinsum{} dispatches the single-tensor reductions and runs in $\less(n^2)$, whereas \numpyeinsum{}, whose multi-operand path is assembled from \texttt{tensordot} calls over shared indices and exposes no genuine unary reduction, forms the $n^4$ outer product.}

\begin{remark}[Why the \Einsum{} libraries contract tensor-wise]
\label{rem:why_tensorwise}
\rev{Two features explain why the \Einsum{} libraries, and the tensor-network community whose tools they descend from, execute contractions tensor-wise rather than index-wise. First, each binary contraction (P1) is itself a \emph{tensor contraction}---an \Einsum{} operation in the sense of Definition~\ref{def:tensor_contraction}, which may sum several indices at once---of which ordinary matrix multiplication is merely the special case of two matrices sharing a single index (Example~\ref{eg:matrix multiplication}). After its contracted and free indices are reshaped into rows and columns, such a contraction is carried out as a matrix multiplication, the single most optimized and GPU-accelerated primitive in numerical computing; contracting tensor-wise lets every step ride this primitive, which is the source of the GPU speed-ups reported in Section~\ref{sec:example_hoif}. Second, the workload of that community is, in a precise sense, the opposite of ours. In quantum-circuit and tensor-network simulation one contracts \emph{very many} tensors over a \emph{large} number of indices $m$, but each index ranges over a \emph{small} dimension (often $n = 2$ for qubits) that may moreover vary across indices; there the dominant difficulty is the NP-hard search for a good contraction ordering, met by elaborate heuristics \citep{markov2008simulating, gray2021hyper}, and the networks are usually \emph{closed-binary}, with each index shared by exactly two tensors. Our $U$/$V$-statistic setting is the reverse: the number of factors $K$ is small, the order $m$ is fixed, and every index ranges over the same large dimension $n$, the sample size, with $n \to \infty$. Consequently the treewidth is a small constant and the exponent of $n$ is the whole story, the optimal ordering can be found once on the small fixed structure and reused (Remark~\ref{rem:ordering}), and hyperedges (an index occurring in three or more factors) and disconnected signatures (the distance-covariance kernel above) are commonplace, placing our problems outside the closed-binary regime. We therefore borrow the tensor-wise execution for its speed while analysing the complexity index-wise; Proposition~\ref{pro:tw_time} shows that the two viewpoints agree in time.}
\end{remark}
