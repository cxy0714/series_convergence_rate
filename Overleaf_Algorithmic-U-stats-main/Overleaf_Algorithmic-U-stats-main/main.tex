...
\title{On computing and the complexity of computing higher-order \pdfu{}-statistics, exactly}

\author[1]{Xingyu Chen\thanks{E-mail: \href{chenxingyu0714@sjtu.edu.cn}{xingyuchen0714@sjtu.edu.cn}}}
\author[2]{Ruiqi Zhang\thanks{E-mail: \href{zrq1706@outlook.com}{zrq1706@outlook.com} 
X. Chen and R. Zhang are co-first authors and alphabetically ordered.}}

\author[1, 3]{Lin Liu\orcidlink{0000-0002-9883-7962}\thanks{E-mail: \href{linliu@sjtu.edu.cn}{linliu@sjtu.edu.cn}. Corresponding author. The authors would like to thank Yixuan Qiu for inspiration and helpful conversations, Shan Luo, Huijuan Ma, Rajarshi Mukherjee, Jamie Robins, Xinwei Shen, Qingyuan Zhao, and participants of the 2025 Pacific Causal Inference Conference (PCIC) and the 2025 Annual Causal Inference Meeting in China for their invaluable feedback. This research is funded by NSFC Grant No. 12471274 and 12090024.}}
...
The remainder of our paper is structured as follows. Section~\ref{sec:preliminaries} introduces and reviews several concepts central to the paper, including $U$/$V$-statistics and tensors (Section~\ref{sec:pre_u_v}), the Einstein summation (Section~\ref{sec:pre_tensor_contraction}), and notions (including vertex elimination, treewidth, and quotient graphs) from graph theory (Section~\ref{sec:graph}). Readers who are familiar with related materials are recommended to skip the relevant subsections. In Section~\ref{sec:alg_u_v}, the new algorithm for exactly computing $U$-statistics is presented. We also illustrate how the algorithm proceeds using concrete examples to facilitate understanding. We then demonstrate the practical merit of our algorithm using three examples in Section~\ref{sec:examples}, including computing HOIF estimators, motif counts in networks (Section~\ref{sec:example_motif}), and computing distance covariances (Section~\ref{sec:dependence}). In our own opinion, Section~\ref{sec:examples} is the most important part of this paper. Section~\ref{sec:conclusions} concludes the paper with a discussion of future directions.
...
In this paper, we have conducted a delicate and systematic study on computing and the complexity of computing higher-order $U$-statistics (and also $V$-statistics), exactly. Explicit connections are established between the complexity of computing higher-order -statistics and concepts from tensor computation and graph theory. An accompanying Python package \package{} is provided to help practitioners compute -statistics more efficiently in applications. % Although Python becomes increasing popular among statisticians \citep{dai2023rehline}, an R interface of \package{} will also be available in due course. 
It will also be interesting to test \package{} on other problems involving $U$-statistics \citep{qiu2024neural, shen2025engression}. 

One limitation of our work is that we prioritize time complexity over space complexity. As a result, when we need to store a very high-order tensor, the sample size $n$ cannot be too large -- this is why we have only evaluated the performance of \package{} on computing HOIF estimators of order $m \leq 7$ when $n = 10000$. Similar limitations also apply to LLM-based foundation models for statistics \citep{hollmann2025accurate}. An important future direction is therefore to explore how to balance time-complexity and space-complexity when the sample size $n$ is large.
...
% The authors would like to thank Jamie Robins, \href{https://scholar.harvard.edu/rajarshi/home}{Rajarshi Mukherjee} and \href{http://www.matteobonvini.com/}{Matteo Bonvini} for their invaluable feedback on our paper and our package \package{}. This research is funded by NSFC Grant No. 12471274 and 12090024 and Shanghai Science and Technology Commission Grant No.21JC1402900.
