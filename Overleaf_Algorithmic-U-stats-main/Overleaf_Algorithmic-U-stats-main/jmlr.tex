...
\ShortHeadings{Chen, Zhang, Liu}{$U$-statistic Computation}
\firstpageno{1}
\setlength {\marginparwidth }{2cm}
...
\author{\name Xingyu Chen\thanks{Equal contribution and alphabetically ordered.} \email xingyuchen0714@sjtu.edu.cn \\
\addr School of Mathematical Sciences \\
Shanghai Jiao Tong University
\AND
\name Ruiqi Zhang$^{*}$ \email zrq1706@outlook.com \\
\addr School of Mathematical Sciences \\
East China Normal University
\AND
\name Lin Liu\thanks{Corresponding author. The authors would like to thank Shan Luo, Huijuan Ma, Rajarshi Mukherjee, Jamie Robins, Xinwei Shen, Qingyuan Zhao, and participants of the 2025 Pacific Causal Inference Conference (PCIC) and the 2025 Annual Causal Inference Meeting in China for their invaluable feedback.} \email linliu@sjtu.edu.cn \\
\addr Institute of Natural Sciences, MOE-LSC, \\
...
Higher-order $U$-statistics are prevalent in various disciplines, including statistics \citep{robins2008higher, bonvini2022fast, bonvini2024doubly, neykov2021minimax, yao2018testing, breunig2024adaptive, sturma2024testing, shao2025u}, machine learning \citep{clemenccon2008ranking, clemenccon2013maximal, shen2025engression}, random graphs \citep{chatterjee2024higher}, Bayesian networks and statistical physics \citep{pearl1988probabilistic, shpitser2011efficient, li2020random}, just to name a few. It is well-known in the statistical community that $U$-statistics are highly time-consuming to compute at large. In the worst-case, an $m$-th order $U$-statistic over a sample of size $n$ takes roughly $O (n^{m})$ steps to compute, because by definition (see Definition~\ref{def:u_stat}) one needs to sum over all $m$-out-of-$n$ permutations. If $n = 10^{4}$ and $m = 7$, $n^{m} \approx 10^{28}$! 

More specifically, in causal inference, \citet{robins2008higher, robins2017minimax} developed a so-called ``higher-order influence function (HOIF)'' framework for constructing nearly rate-optimal estimators of average treatment effects under ignorability under various structural assumptions, using higher-order $U$-statistics with order roughly of $\sqrt{\log n}$. Despite their appealing statistical properties, implementing such complex estimators remains challenging in practice and computational complexity is one important bottleneck. Here we quote a sentence from a recent graduate-level textbook on causal inference by \citet{wager2024causal} (page 44) to highlight such a quandary:
\begin{quote}
\emph{One challenge with the HOIF estimator of \citet{robins2017minimax}, however, is that to date it has been challenging to implement in practical applications ...}
\end{quote}
Such criticism is also echoed in a very recent review article (Section 3.5.3) on the future of causal inference \citep{cinelli2025challenges}:
\begin{quote}
\emph{In the relatively few settings where estimators are available that can improve on doubly robust style methods (for example, using higher-order methods ... the estimators are often computationally intensive and require careful tuning. There are many opportunities to make these methods more practical, automatic, and user-friendly.}
\end{quote}

This paper was initially motivated by (at least partially) addressing the computational aspect of this challenge, but we will present general-purpose theory and algorithm for computing general higher-order $U$-statistics beyond those for HOIF estimators. We will revisit HOIF estimators in Section~\ref{sec:example_hoif}. 

To tackle the difficulty of computing higher-order $U$-statistics, statisticians and computer scientists have long considered the so-called ``incomplete $U$-statistics'', where one sums over only a random subset of all $m$-out-of-$n$ permutations \citep{blom1976some, janson1984asymptotic, nowicki1988subgraph, shao2025u}. Another natural approach is to choose a subsample of size $n' < n$ from the whole sample and then compute a complete $U$-statistic from that subsample. Both strategies are akin to randomized or sketching algorithms in the theoretical computer science literature \citep{mahoney2011randomized}. The randomization approach certainly injects extra randomness in the final statistical results, which may produce inconsistent statistical conclusions across different randomly selected permutations/subsamples \citep{guo2025rank}. For a restricted class of higher-order $U$-statistics with decomposable and symmetric (or more precisely permutation-invariant) kernels, \citet{kong2018estimating} cleverly recognize that exact computation can be accomplished by repeated matrix multiplications. For asymmetric kernels, their strategy essentially computes an incomplete $U$-statistic instead. A somewhat similar strategy has also been considered in \citet{zhang2025adaptive}, in which an incomplete $U$-statistic with a special kernel structure is computed more efficiently via dynamic programming. All the above approaches may suffer from efficiency loss by not using all the available information.

The literature on (the complexity of) exactly computing higher-order $U$-statistics is rather scarce. This is hardly surprising because the exact time complexity of computing higher-order $U$-statistics seems to simply equal the total number of summations. However, as we will demonstrate in the paper, exactly computing $U$-statistics can be improved compared to a brute-force summation over all summands (using for-loops) when the kernel enjoys a particular decomposable structure specified in Definition~\ref{def:mul_decomp} later. Interestingly, as we will exhibit in Section~\ref{sec:examples}, such a structure holds in many statistical applications. It will also be clear later that it is helpful to bring in tools such as the Einstein summation from the computational mathematics community to obtain improved algorithms of computing $U$-statistics by leveraging such a structure. It is worth noting that Section~3 of \citet{liu2024hypothesis} considered to use di-graphs to represent the structure of a class of $U$-statistics, a strategy conceptually similar to ours. Yet our new algorithm is designed to cover general higher-order $U$-statistics.
...
\begin{itemize}
\item We design a new algorithmic framework for exactly computing higher-order $U$-statistics and another closely related objects called $V$-statistics by leveraging the Einstein summation operation and related concepts from combinatorics and graph theory. Thanks to the connection with graph theory, we can provide an optimistic estimate of the time complexity of exactly computing $U$-statistics (see Section~\ref{sec:alg_u} for details). Although the idea is conceptually simple, to the best of our knowledge, it has not been thoroughly explored in the existing statistical literature.

\item Despite the ubiquity of $U$-statistics (see later this section), a dedicated, general-purpose software package for computing them seems to be lacking. To fill in this void, we developed a Python package called \package{}, which implemented the aforementioned algorithm. We demonstrate that the runtime of \package{} achieves the state-of-the-art performance in various examples, including computing HOIF estimators, motif counts of networks, and the distance covariance in the independence testing problem. In fact, as will be clear in Section~\ref{sec:examples}, \package{} can dramatically improve the runtime compared to popular benchmarks. \package{} currently supports both CPU and GPU infrastructure. Having access to GPU can further improve the runtime because our algorithm heavily utilizes tensor operations such as the Einstein summation. We envision that the package \package{} can be a valuable tool for practitioners.
\end{itemize}

\begin{remark}
\label{rem:misc}
We focus on scalar-valued $U$-statistics in this paper, but we plan to upgrade our package {\normalfont\package{}} to handle vector- or matrix-valued cases in its next version. In this paper, one floating point operation is considered as the basic computation unit.
\end{remark}
...
In this section, we first collect some frequently used notation. We adopt the following convention: we use the demarcation $\{\}$ for a set, representing an unordered collection of distinct elements; whereas we use the demarcation $()$ for a tuple, meaning an ordered collection of elements which may contain duplicates. Given a positive integer $m$, we let $[m] \coloneqq \{1, 2, \ldots, m\}$. To reduce clutter, we write $\bar{i}_{m} \equiv (i_{1}, \cdots, i_{m})$ for short. $\naturals$ denotes the set of natural numbers including zero, while $\positivenaturals$ denotes the set of all positive integers. Given a tuple $\bm{s}$, we let $\takesetp{\bm{s}} \coloneqq \{s_k \mid k\in [m]\}$ denote the set containing all elements appear in $\bm{s}$. Given a non-empty and finite set $S$ and a positive integer $m$, all $m$-permutations of elements in $S$ is denoted by $\perm{S, m}$. All partitions of $S$ is denoted by $\partitionp{S}$. If $S$ happens to be integer set $[m]$, $\partitionp{S}$ is simplified to $\partitionp{m}$. Given sets $S_{1}, \cdots, S_{m}$, $\times_{i = 1}^{m} S_{i}$ denotes their Cartesian product. When $S_{1} = \cdots = S_{m} = S$, we simply write $S^{m} \equiv \times_{i = 1}^{m} S_{i}$. $S^* \coloneqq \cup_{m \in \positivenaturals} S^m$ denotes all tuples possibly generated by $S$. Given a tuple $\bm{s} \in S^m$ and a tuple $\alpha \in [m]^*$, we let $\bm{s}[\alpha] \coloneqq (s_
...
Terminologies from graph theory will be frequently encountered. Throughout this paper, all graphs are assumed to be finite and undirected without self-loops, but are allowed to have isolated vertices, unless stated otherwise. A graph $G$ is denoted by a pair $(V, E)$, where $V \equiv V (G)$ is the finite vertex set, and $E \equiv E (G)$ is the edge set of $G$. Two vertices $u$ and $v$ are said to be adjacent if $\{u, v\} \in E$. A complete graph is a graph with every pair of vertices connected by an edge, and a complete graph with $m$ vertices is denoted by $\completegraphp{m}$. For any vertex $v \in V$, its neighbor set is denoted as $N_G (v) \coloneqq \{ u \in V \mid \{u, v\} \in E \}$, and the degree of $v$ is defined as $\degp{v}{G} \coloneqq |N_G(v)|$. A graph $G^\prime$ is a subgraph of graph $G$, if $V (G^\prime) \subseteq V (G)$ and $E (G^\prime) \subseteq E (G)$. For any subset $V' \subseteq V$, the induced subgraph $G_{V'}$ has vertex set $V'$ and edge set $E_{V'} \coloneqq E (G_{V'}) \equiv \{\{u, v\} \in E \mid \forall \, u, v \in V'\}$. A subset of vertices $C \subseteq V$ is said to be a clique if $G_{C}$ is complete.
...
The remainder of our paper is structured as follows. Section~\ref{sec:preliminaries} introduces and reviews several concepts central to the paper, including $U$/$V$-statistics and tensors (Section~\ref{sec:pre_u_v}), the Einstein summation (Section~\ref{sec:pre_tensor_contraction}), and notions (including vertex elimination, treewidth, and quotient graphs) from graph theory (Section~\ref{sec:graph}). Readers who are familiar with related materials are recommended to skip the relevant subsections. In Section~\ref{sec:alg_u_v}, the new algorithm for exactly computing $U$-statistics is presented. We also illustrate how the algorithm proceeds using concrete examples to facilitate understanding. We then demonstrate the utility of our algorithm using three examples in Section~\ref{sec:examples}, including computing HOIF estimators, motif counts in networks (Section~\ref{sec:example_motif}), and computing distance covariances (Section~\ref{sec:dependence}). In our own opinion, Section~\ref{sec:examples} is the most important part of this paper. Section~\ref{sec:conclusions} concludes the paper with a discussion of future directions.

\section{Preliminaries}
\label{sec:preliminaries}

In this section, we will review concepts important for later development, including $U$/$V$-statistics and tensors (Section~\ref{sec:pre_u_v}), the Einstein summation operation (Section~\ref{sec:pre_tensor_contraction}), and useful notions from graph theory (Section~\ref{sec:graph}). The way that we define $U$- or $V$-statistics may look strange to statisticians, but we find such definitions more convenient to work with when combined with the Einstein summation formulation.
...
\subsection{{\normalfont\Einsum{}} Notation and {\normalfont\Einsum{}} Operation}
\label{sec:pre_tensor_contraction}

As mentioned, our algorithm to be revealed will rely on the Einstein summation operation (abbreviated as the \Einsum{} operation) over tensors. In this section, we give a brief but (hopefully) pedagogical overview for readers unfamiliar with these related concepts. We also steer readers to \citet{klaus2023compiling} or \citet{blacher2024einsum} for more details. % We mention in passing that it is possible to write our paper using the tensor (hyper)network language. But we decide not to do so because our paper targets more on statisticians.

The \Einsum{} operation to be defined serves as a fundamental computational primitive in our proposed algorithm for efficiently and exactly computing $U$/$V$-statistics. The \Einsum{} operation generalizes the classical Einstein summation convention introduced in \cite{einstein1916grundlage}. The modern \Einsum{} operation, now implemented in popular Python packages such as \numpy{} and \torch{}, can be used for efficient tensor computation. To explain the \Einsum{} operation, we first introduce a concept called the \Einsum{} notation.

\begin{definition}[{\normalfont\Einsum{}} Notation]\label{def:tensor_expression}
...
It is noteworthy that in the above definition, the index set $[m]$ is simply chosen for notational convenience and in principle the indices do not have to be integers or even numbers. We now define the \Einsum{} operation based on the \Einsum{} notation.
...
We now use several concrete examples to illustrate the above definitions.
...
\begin{equation*}
T^{\rm out} (i, j) = \sum_{k = 1}^n T_{1} (i, k) T_{2} (k, j). 
\end{equation*}
...
\end{align*}
so the claim holds by identifying $\alpha' [2] = a$, $\alpha' [3] = b$, and $\alpha' [4] = c$.
\end{example}
...
\begin{remark}
\label{rem:Python}
The \Einsum{} operation has been implemented in highly optimized Python libraries such as {\normalfont\numpyeinsum{}} and {\normalfont\torcheinsum{}}. These efficient algorithmic subroutines, enabling efficient \Einsum{} operation in practice, serve as the building blocks for our algorithm. Notably, developed for training large overparameterized neural networks in tensor forms, {\normalfont\torcheinsum{}} is also designed to leverage GPU computing to further enhance the scalability and speed of the \Einsum{} operation, as we will demonstrate using an example from statistics in Section~\ref{sec:example_hoif}.
\end{remark}

\begin{remark}
\label{rem:tensor networks}
The \Einsum{} operation is frequently employed in scientific computing problems. For example, the \Einsum{} operation serves as a basic algorithmic subroutine for computation over tensor (hyper)networks, a useful mathematical language adopted by the quantum computing and quantum information community \citep{markov2008simulating, stoudenmire2018learning}. % The tensor network formulation can help visualize summations (partial traces) over tensors in the language of graphs (e.g. Figure~\ref{fig:tensor_network_example} visualizing Example~\ref{eg:tensor}). 
...
In this section, we recall three key concepts from graph theory that are central to our complexity-theoretic characterization: \emph{vertex elimination} 
% , which helps us represent tensor contraction along a single index; 
and \emph{treewidth}, which captures the dominant computational complexity; and \emph{quotient graph}, which is related to the decomposition of a $U$-statistic into a linear combination of $V$-statistics. Here we adopt the terminologies from \citet{bodlaender2010treewidth}.
...
\section{A New Algorithm of Exactly Computing \pdfu{}-Statistics}
\label{sec:alg_u_v}

We are ready to present our new algorithm for exactly computing $U$-statistics, along with an analysis of the time complexity. Our algorithm has been implemented in an open source Python package \package{}. 

We begin by introducing the \emph{multiplicative-decomposable} (MD) structure of the $U$/$V$-statistic kernel in Section~\ref{sec:decomposition}. We then describe our algorithm and the complexity analysis for $V$-statistics in Section~\ref{sec:alg_v}, followed by those for $U$-statistics in Section~\ref{sec:alg_u}.
...
The \emph{multiplicative-decomposable} (MD) structure is the key assumption we impose on the kernel of the $U$-statistic to be computed. Such a structure is essential for developing more efficient algorithms for exactly computing $U$/$V$-statistics than trivially summing over all $m$-out-of-$n$ permutations/$[n]^{m}$. Similar concepts are commonly exploited in developing numerical algorithms for solving ordinary or partial differential equations in the scientific computing literature.

\begin{definition}[Multiplicative Decomposition]\label{def:mul_decomp}
Let $\samplespace$ be a non-empty set, $m$ a positive integer, and $h: \samplespace^m \to \bbR$. Suppose $\calA \coloneqq (A_k)_{k=1}^K \in [m]^{**}$ satisfies $\bigcup_{k=1}^K \takesetp{A_k} = [m]$, and let $\calH \coloneqq (h_k)_{k=1}^K$ be a tuple of functions where each $h_k$ is defined on $\samplespace^{|A_k|}$. $\calH \times \calA$, comprised of ordered pairs of the form $(h_{k}, A_{k})$ for $k \in [K]$, is called a multiplicative decomposition of the function $h$, if for any $\bm{x} \in \samplespace^m$, 
\begin{align*}
h(\bm{x}) = \prod_{k=1}^K h_k\big(\bm{x}[A_k]\big).
\end{align*}
In the sequel, we refer to the function tuple $\calH$ as the kernel components and the index tuple $\calA$ as the decomposition signature. For convenience, we say that a kernel $h$ permits a (nontrivial) MD structure $\calH \times \calA$ if $K > 1$.
\end{definition}

$U$-statistic kernels with MD structures are pervasive in statistical applications. We will provide three examples in Section~\ref{sec:examples}.
...
\end{align*}
Then following Definition~\ref{def:v_stat} and Definition~\ref{def:mul_decomp}, we have
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Ensure A tuple of tensors $\calT \coloneqq (T_k: [n]^{m_k} \to \reals)_{k=1}^K$, where $T_k$ is the tensorized version of $h_k$
...
\subsection{The Algorithm and Time Complexity of Computing \pdfv{}-Statistics}
\label{sec:alg_v}
...
\end{subfigure}
\caption{Examples of decomposition graphs associated with different decomposition signatures.}
...
\subsection{The Algorithm and Time Complexity of Computing \pdfu{}-Statistics}
\label{sec:alg_u}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Require 
        \Statex An \Einsum{} notation $\calA = (A_k)_{k=1}^K$ with index set $[m]$ 
        \Statex A partition $\sPi \in \partitionp{m}$ of size $J$
...
Then it holds that
\begin{equation*}
\ustat(\calT, \calA) = \ustat(\tilde{\calT}, \calA).   
\end{equation*}
\end{lemma}
...
With the sparsification trick, the final algorithm of computing $U$-statistics is displayed in Algorithm~\ref{alg:u_stats} below. 
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{Ustats}{$\calT, \calA; \bm{X}$}
            \State Let $\widetilde{\calT}$ be constructed as in  Lemma~\ref{lem:sparsification_trick}  
...
\begin{align*}
M  & \coloneqq \max\{\treewidthp{G_\calA /\pi} \mid \pi \in \partitionp{m}^\calA \}, \\
N_{l} & \coloneqq |\{\pi \in \Pi_{m}^{\calA} \mid \treewidthp{G_{\calA} / \pi} = l\}|, \; \forall \, l 
\in [M].
\end{align*}
...
In the above result, quantities $N_{l}$ and $M$ are introduced. $N_{l}$ is the number of partitions in $\Pi_{m}^{\calA}$ such that the treewidth of the decomposition graph $G_{\calA}$ quotient by $\pi$ is $l$. Although abstract, $N_{l}$ and $M$ can in principle be computed for concrete examples. For instance, in Section~\ref{sec:example_hoif}, we report the values of $M$ for HOIF estimators between orders $2$ to $12$; see the last column of Table~\ref{tab:HOIF}. At this moment, we are not yet aware of a good strategy of counting $N_{l}$, except brute-force enumeration or a loose upper bound by the total number of $V$-statistics in the decomposition; see the third column of Table~\ref{tab:HOIF}.
\end{remark}
...
\begin{remark}
\label{rem:tw}
In this paper, we have used the treewidth of the decomposition graph (or its quotient as in Corollary~\ref{cor:U}) to characterize the optimistic time-complexity of exactly computing $U$/$V$-statistics, and the \Einsum{} operation. Although treewidth has been a widely used concept for establishing the time-complexity of algorithms on graphs \citep{dong2021nearly}, it is in general nontrivial to compute or derive a tight bound. To our knowledge, Theorem~4.7 in \cite{kneis2009bound} proved a bound for treewdith linear in the edge size. We do not know if this bound can be further improved. We will discuss this as an open problem at the end of this paper (Section~\ref{sec:conclusions}).
\end{remark}

Before moving on to applications of our algorithm in the next section, we summarize the steps of exactly computing a $U$-statistic using our proposed algorithm. Given a $U$-statistic kernel $h$ and a sample $\bm{X}$:
...
In this section, we evaluate the performance of our new algorithm for computing $U$/$V$-statistics exactly in three concrete statistical applications, including computing higher-order influence functions (Section~\ref{sec:example_hoif}), computing motif counts in random graphs (Section~\ref{sec:example_motif}) and computing distance covariance measures (Section~\ref{sec:dependence}). % At the end of Section~\ref{sec:dependence}, using the distance covariance measure, we explain how the heuristic algorithms from \opteinsum{}, currently implemented in \package{}, find certain good but suboptimal \Einsum{} ordering. The performance corresponding to the optimal \Einsum{} ordering will also be presented.

As mentioned, we have made our algorithm available for public use as a Python package \package{}, which is available to download from either its GitHub repository (clicking \package{}) or the Python package index \href{https://pypi.org/}{\texttt{PyPi}} by typing the following on the command line:
\begin{lstlisting}[language=bash]
pip install u-stats
\end{lstlisting}
\noindent We also refer interested readers to the GitHub link for how to implement \package{} in practice.
...
\end{align}
Then obviously $h_j^{(m)}$ permits a MD structure by letting $f_{l} (X_1, X_2) \coloneqq A_1 \phi (Z_1)^{\top} A_2 \phi (Z_2)$ for $l < j$ and $f_{j} (X_1, X_2) \coloneqq A_1 \phi (Z_1)^{\top} \phi (Z_2) Y_2 $. The corresponding decomposition signature of $h_j^{(m)}$ is $((i, i+1))_{i=1}^{j-1}$, which is denoted by $\calA_{\mathsf{HOIF},j}$, so the decomposition graph of $\calA_{\mathsf{HOIF},j}$ is a chain graph with $e = j - 1$ edges.

We also demonstrate the effectiveness of the sparsification trick using the HOIF example (see Lemma~\ref{lem:sparsification_trick}). The following table enlists the total number of $V$-statistics and the number of floating-point operations (FLOPs) that need to be computed before and after applying the sparsification trick when the sample size is $10^4$. The rightmost column of Table~\ref{tab:HOIF} also records the value of $M$ defined in Corollary~\ref{cor:U}. The derivation of this column can be found in Appendix~\ref{app:HOIF}. 
...
In Tables~\ref{tab:HOIF_CPU} and \ref{tab:HOIF_GPU}, we report the average runtime over 3 runs when computing the $U$-statistic $\ustatp{h_j^{(m)}}{\bm{X}}$ where kernel function $h_j^{(m)}$ is as in \eqref{eq:hoif_kernel} with $m=7$ and $j \in \{2, 3, \cdots, 7\}$ and sample size $n \in \{1000, 2000, 4000, 8000, 10000\}$, respectively using CPU parallel computation and using a single GPU. We would like to highlight that even when $m = 7$, $n = 10000$, it only takes about 5 minutes using our new package \package{}, with CPU parallel computation. If one instead has access to a single GPU, the runtime even reduces to about 12 seconds, a substantial speedup! We remark in passing that all the runtime reported in Table~\ref{tab:HOIF_CPU} and Table~\ref{tab:HOIF_GPU} excludes the time of kernel evaluation and tensorization.


\begin{table}[ht]
\centering
\caption{Average runtime (in seconds) using CPU parallel computation. Experiments were conducted on Intel Xeon ICX Platinum 8358 CPUs (2.6GHz, 64 total cores) with memory of 512 GB, evaluated across varying sample sizes and orders of HOIF-type $U$-statistics.}
\label{tab:HOIF_CPU}
\begin{tabular}{cccccc}
\toprule
...
\subsection{Application 2: Motif Counts in Random Graph Models and Networks}
...
Interestingly, motif counts can be equivalently represented as $U$-statistics. Leveraging this observation, we show that various motif counts can be computed using a single \Einsum{} operation. While this approach may not capture fine-grained structural information such as sparsity of the adjacency matrix, it is highly efficient in the context of dense graphs, significantly outperforming a popular software called \igraph{} designed for computing motif counts.

\iffalse
To illustrate the general approach of formulating motif counts using $U$-statistics, we begin with the example of counting triangles, i.e., motif $\sfr_2$ in Figure~\ref{fig:motif-3-4}. Let $G$ be a simple graph with adjacency matrix $A \in \{0,1\}^{n \times n}$ and $n$ vertices. A classical expression for the number of triangles is given by

\begin{equation}
\label{equ:motif_triangle_1}
    \motifrg{R_2}{G} = \frac{1}{6} \mathsf{Tr}(A^3),
\end{equation}

where $\mathsf{Tr}(A^3)$ denotes the trace of $A^3$. Expanding this expression, we obtain

\begin{equation*}
    \motifrg{R_2}{G} = \frac{1}{6} \sum_{1 \le i_1,i_2,i_3 \le n } A_{i_1,i_2} A_{i_2,i_3} A_{i_3,i_1}.
\end{equation*}

Since $G$ is a simple graph, the diagonal elements of $A$ are all zero. Thus, the above summation can be rewritten by a $3$-order $U$-statistics :

\begin{equation*}
    \motifrg{R_2}{G} = \frac{1}{6} \sum_{1 \le i_1 \ne i_2 \ne i_3 \le n } A_{i_1,i_2} A_{i_2,i_3} A_{i_3,i_1}
    \eqqcolon \sum_{ \alpha \in \perm{n,m}} h_{R_2} ( \alpha ),
\end{equation*}

where the kernel function is defined as $h_{R_2}(i_1, i_2, i_3) = A_{i_1,i_2} A_{i_2,i_3} A_{i_3,i_1}$.
\fi
...
\iffalse
For an $m$-vertex motif $\sfr$ with adjacency pattern $M_R \in \{0,1\}^{m \times m}$, the kernel function is defined as:
\begin{equation*}
h_{\sfr} (\bar{i}_{m}; A) = \prod_{1 \le s < t \le m} A_{i_s,i_t}^{M_R(s,t)} \cdot (1 - A_{i_s,i_t})^{1 - M_R(s,t)}.
\end{equation*}
This evaluates to $1$ if the induced subgraph on $(i_1, \dots, i_m)$ is isomorphic to the motif $R$, and $0$ otherwise. 
\fi
\begin{equation*}
...
To be concrete, letting $B \equiv 1 - A$, the corresponding $U$-statistics for motif counts for specific $3$-vertex motifs and $4$-vertex motifs (see Figure~\ref{fig:motif-3-4} for which motifs $\sfr_1$ to $\sfr_8$ encode) read as follows:
\begin{align*}
...
% Therefore, motif counts for $m$-vertex motifs naturally correspond to a complete decomposition signature $\calA_{\mathsf{Motif},m} = ((s,t))_{1 \le s < t \le m}$, where the decomposition graph is always the complete graph $\completegraphp{m}$. This structure leads to two consequences: (1) a positive one—thanks to our diagonal exclusion strategy, only one term needs to be computed; and (2) a limiting one—we cannot reduce the exact computational complexity of motif counting below $\less(n^m)$, due to the complete structure of the kernel.

We compare our package \package{} with \igraph{} \citep{csardi2006igraph} on exactly computing all 3-vertex and all 4-vertex motif counts. \igraph{} computes motif counts by using a highly optimized algorithm based on \citet{wernicke2006fanmod}. 

We generate random graphs using the classical Erd\"{o}s-R\'{e}nyi model $G (n, p)$, where each of the $\binom{n}{2}$ possible edges between $n$ vertices is present independently with probability $p$. We report the runtime comparisons in Table~\ref{tab:motif_counts_3} and Table~\ref{tab:motif_counts_4}. All experiments were conducted on a server equipped with Intel Xeon Platinum 8358 CPUs (2.6GHz, 64 cores) and memory of 512 GB. Our implementation makes full use of all 64 CPU cores via multiprocessing, while \igraph{} runs in a single-threaded setting, due to its current implementation limitation. 

To quantify the performance, we consider two metrics:
\begin{itemize}
\item \textbf{Speedup}: the ratio of \igraph{} runtime to the runtime of \package{} with parallel computation;
\item \textbf{Single-core speedup}: a very rough estimate of per-core speedup, computed as the speedup divided by the number of cores (i.e. $64$).
\end{itemize}

As shown in Table~\ref{tab:motif_counts_3} and Table~\ref{tab:motif_counts_4}, \igraph{} has better runtime performance than \package{} when $p$ is small, corresponding to a sparse graph. Because \package{} is designed as a general purpose package for computing $U$-statistics, we do not further take into account the potential sparsity of the adjacency matrix. When $p$ increases (i.e., as the graph becomes denser), the runtime of \igraph{} grows considerably even when $p$ is as low as $0.1$. Yet \package{} still maintains roughly the same runtime throughout, which is much faster than that of \igraph{} when $p$ grows.
...
\centering
\caption{
Runtime comparison of exact all $3$-vertex motif counts using \package{} and \igraph{} on Erd\"{o}s-R\'{e}nyi graphs $G(n, p)$ with $n = 5623$. 
Experiments were run on Intel Xeon ICX Platinum 8358 CPUs (2.6GHz, 64 total cores) with memory of 512 GB.
“Speedup ratio” compares \igraph{} to our parallel method; “single core speedup” assumes single-core execution.
}

\label{tab:motif_counts_3}
\begin{tabular}{ccccc}
\toprule
\makecell{\textbf{Edge} \\ \textbf{Prob.} $p$} & \makecell{\package{} \\ \textbf{Time (s)}} & \makecell{\textbf{\igraph{}} \\ \textbf{Time (s)}} &\makecell{\textbf{Speedup} \\ \textbf{Ratio}} &  \makecell{\textbf{Single-Core} \\ \textbf{Speedup}} \\
\midrule
...
\centering
\caption{
Runtime comparison of exact all $4$-vertex motif counts using \package{} and \igraph{} on Erd\"{o}s-R\'{e}nyi graphs $G(n, p)$ with $n = 2000$. 
Experiments were run on Intel Xeon ICX Platinum 8358 CPUs (2.6GHz, 64 total cores) with memory of 512 GB.
“Speedup ratio” compares \igraph{} to our parallel method; “single core speedup” assumes single-core execution.
}
...
\begin{tabular}{ccccc}
\toprule
\makecell{\textbf{Edge} \\ \textbf{Prob.} $p$} & \makecell{\textbf{\package{}} \\ \textbf{Time (s)}} & \makecell{\textbf{\igraph{}} \\ \textbf{Time (s)}} &\makecell{\textbf{Speedup} \\ \textbf{Ratio}} &  \makecell{\textbf{Single-Core} \\ \textbf{Speedup}} \\
\midrule
...
\subsection{Application 3: Distance Covariance Measures for Independence Testing}
...
\end{align*}
where $h_1 (O_i, O_j) \coloneqq \|X_i - X_j\|_2$ and $h_2 (O_i, O_j) \coloneqq \|Y_i - Y_j\|_2$ for $i, j \in [n]$. Here $\|\cdot\|_2$ denotes the vector $\ell_2$-norm. 

Quoting from Table~5 of \citet{shao2025u}, it takes ``8099.73 seconds'' to compute $\mathsf{dCov}^2$ among 11 random vectors, together with its confidence interval, when $n = 138$. Each vector corresponds to a different stock sector—such as Health Care or Industrials—and its components represent the monthly log-returns of individual stocks within that sector, using historical S\&P 500 data. To bypass such a high computational burden, \citet{shao2025u} resort to computing a randomized incomplete $U$-statistic instead. In the same Table~5, \citet{shao2025u} demonstrated that there is indeed a significant speedup when using randomized incomplete $U$-statistics. It is noteworthy that another motivation of \citet{shao2025u} to use randomized incomplete $U$-statistic is due to its asymptotic normality \citep{chen2019randomized} even when the original complete $U$-statistic has a Gaussian chaos limiting distribution. But we will not further discuss this issue as it is not directly relevant to the main theme of our paper.
...
In this paper, we have conducted a delicate and systematic study on computing and the complexity of computing higher-order $U$-statistics (and also $V$-statistics), exactly. Explicit connections between the complexity of computing higher-order $U$/$V$-statistics and concepts from tensor computation and graph theory are established. An accompanying Python package \package{} is provided to help practitioners compute $U$/$V$-statistics more efficiently in applications. Although Python becomes more and more popular among statisticians \citep{dai2023rehline}, an R interface of \package{} will also be available in due course. It will also be interesting to test \package{} on other problems involving $U$-statistics \citep{qiu2024neural, shen2025engression}. 

One limitation of our work is that we prioritize time complexity over space complexity. As a result, when we need to store a very high-order tensor, the sample size $n$ cannot be too large -- this is why we have only evaluated the performance of \package{} on computing HOIF estimators with order $m \leq 7$ when $n = 10000$. An important future direction is thus to explore how to balance the time-complexity and space-complexity when the sample size $n$ are large.

Finally, we conclude our paper with an open problem that we ourselves find intriguing. As we indicated in Remark~\ref{rem:tw} at the end of Section~\ref{sec:alg_u_v}, the treewidth of the decomposition graph associated with the \Einsum{} notation of the underlying $V$-statistic relates to its time complexity. But the treewidth of a graph is not easy to compute either analytically \citep{harvey2018treewidth} or numerically \citep{arnborg1987complexity}. It will be interesting to get a sharp bound on treewidth using the number of vertices or edges of the graph. As mentioned in Remark~\ref{rem:tw}, \citet{kneis2009bound} has proved that the treewidth is bounded at most linearly by the edge size, or more precisely bounded by $e / 5.769 + O (\log n)$ where $n \coloneqq |V|$ denotes the number of vertices in $G$. Yet we do not know if this bound can be further improved. Below we provide the maximum tree-width possible for edge size no greater than $15$, to hopefully serve as a starting point of fully addressing this problem.
...
\end{equation*}
\end{observation}
\noindent The proof can be found in Appendix~\ref{app:treewidtg_table}. However, extending the above result to $e > 15$ is currently beyond our reach.
...
\section{Proof of Observation~\ref{pro:treewidth_table} and Results in Section~\ref{sec:example_hoif}}
\label{app:treewidth_table_hoif_results}

\subsection{Proof of Observation~\ref{pro:treewidth_table}}
...
\end{equation*}
This map can be interpreted as a coloring of the vertices: each vertex $k$ is colored by the index of the subset $B_i$ which it belongs to, so that all vertices in $Q_i$ are dyed by color $i$. By above analysis, each pair of adjacent vertices is dyed by different colors, thus for each $i \in [6]$, $ \{ Q_{c(i)}, Q_{c(i+1)} \}$ is an edge in $\graphform{\calA_7} / \pi^*$.

Then, the sequence of edges
\begin{equation*}
\{ Q_{c(1)}, Q_{c(2)} \}, \ 
\{ Q_{c(2)}, Q_{c(3)} \}, \ 
\{ Q_{c(3)}, Q_{c(4)} \}, \ 
...
\{ Q_{c(6)}, Q_{c(7)} \}
\end{equation*}
corresponds to an Euler trail (i.e., a sequence of edges in which consecutive edges share a common endpoint, visiting every edge of the graph exactly once without repetition.) in the quotient graph $\graphform{\calA_7} / \pi^* = \completegraphp{4}$.
