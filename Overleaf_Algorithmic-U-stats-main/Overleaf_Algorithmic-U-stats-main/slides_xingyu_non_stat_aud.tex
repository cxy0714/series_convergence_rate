...
\newcommand{\cugraph}{\libref{cugraph}{https://docs.rapids.ai/api/cugraph/legacy/api_docs/api/cugraph/cugraph.triangle_count/}}
% simplified command
...
Website:  \href{https://cxy0714.github.io/}{\texttt{https://cxy0714.github.io/}}
}

\institute{\normalsize 上海交通大学首届全国高层次博士生学术论坛 \\
}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Given a kernel function $h(x_1, \ldots, x_m): \calX^m \to \bbR$ and a sequence of samples $X_1, \ldots, X_n$. 
        \item The U-statistic takes the form
              \begin{align*}
                  \mathbb{U}_{n,m}[h] =  \sum_{1 \le i_1 \neq i_2 \neq \cdots \neq i_m \le n} h(X_{i_1}, \ldots, X_{i_m}).
              \end{align*}
        \item The V-statistic takes the form
              \begin{align*}
                  \mathbb{V}_{n,m}[h] =  \sum_{1 \le i_1,i_2,\cdots,i_m \le n} h(X_{i_1}, \ldots, X_{i_m}).
              \end{align*}
        \item Two problem in computation:
            \begin{center}
                \footnotesize
                \begin{tabular}{lcc}
                  \toprule
                  ...
                  \textbf{Highly optimized library?} \\
                  \midrule
                Matrix multiplication ($n\times n$) &
                  $O(n^3), O(n^{2.48})$\footnote{\cite{strassen1986asymptotic}},$O(n^{2.372})$\footnote{\cite{williams2024new}},...  &
                  \textcolor{blue}{ Yes, \href{https://www.netlib.org/blas/}{BLAS}, \href{https://libeigen.gitlab.io/}{Eigen},... } \\ [0.8ex]
                    $m$-th order $U$-statistic  &
                  (naively) $O(n^m)$ &
                  \textcolor{red}{ No, but will use \texttt{Einsum}} \\
                  \bottomrule
                \end{tabular}
              \end{center}
    \end{itemize}
...
\begin{frame}
    \frametitle{Example 1: Dependence Measures}
    \begin{itemize}
        ...
        \item The complexity will be {\color{red} $ O(n^2)$} given the $n \times n$ matrices $a_{ij},b_{ij}$:

        \item In \cite{shao2025u}, randomized select $O(n^\alpha)$ pairs to approximate.
\begin{table}[htbp]
\centering
\caption{
Runtime (in seconds) comparison of various methods for computing $\mathsf{dCov}^{2}$. 
Experiments were run on Intel Xeon ICX Platinum 8358 CPUs (2.6GHz, 64 total cores) with memory of 512 GB. $n = 138$.
}
\label{tab:dcov}
\scalebox{0.85}{
...
\begin{frame}
\frametitle{Example 2: Motif Counts}
\begin{itemize}
...
\begin{frame}
    \frametitle{Example 3: U-Statistics in Causal Inference: HOIFs}
    \begin{itemize}
    ...
    \item For complexity, given the $n \times n$ matrices $A^{(k)}_{ij} = f_k(X_i, X_j)$ for $k = 1, \cdots, m-1$:
    \begin{equation*}
    \begin{split}
...
\begin{frame}
    \frametitle{Algorithm Framework : Sparsification}
...
\begin{frame}
\frametitle{ Complexity: $V$-statistics}
    For {\bf  Complexity of  $V$-statistics}:
    \begin{itemize}
    ...
    & = \sum_{i_1,i_2,i_3} \tilde{T}^{(1)}_{i_1i_2} \tilde{T}^{(2)}_{i_2i_3}  - \sum_{i_1, i_2 } \tilde{T}^{(1)}_{i_1i_2} \tilde{T}^{(2)}_{i_2i_1} \\
        \end{align*}
    \item Consider the first $V$-statistic, with  $\calA_{\pi_1} = ((1,2),(2,3)).$
\begin{columns}[t] 
...
\setlength\itemsep{1.5em}
        \item In general, the complexity will depend on the computational ordering of indexes.

        \item A simple graph $G_{\calA}$ captures the decomposable structure $\calA$.
        \begin{figure}
        \centering
        \includegraphics[width=1.0\linewidth]{Figures_slides/graph_u.png}
    \end{figure}

    \item Summing over one index corresponding to \textbf{eliminate} the vertex in graph.
    \end{itemize}
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures_slides/procedure.png}
    \caption*{Procedure of Computing a 3-th order V-statistic with $\calA = ((1,2), (2,3))$}
\end{figure}
\end{frame}
...
\hline
\textbf{Decomposition Graph} & \textbf{Elimination Ordering} & \textbf{Treewidth} & \textbf{Complexity} \\
\hline
...
\end{tabular}
\caption{Decomposition graphs, elimination orderings, and treewidths.}
\label{tab:graph_ordering_treewidth}

 \textbf{treewdith} of a simple graph $G$ is denoted by $\operatorname{tw}(G)$. Let $V(G) = 
            [m] = \{1,2,\cdots,m\}$, then:
            \begin{align*}
                \operatorname{tw}(G) = \min_{\sigma \in \perm{[m]}} \max_{ i \in [m]} \deg (i, G_{i-1} ), G_0 = G, G_i= \eliminatevg{i}{G_{i-1}}.
            \end{align*}
\end{table}
...
\setlength\itemsep{1.5em}
   \begin{mylemma}[C., Zhang, Liu, 25]
                  \label{lem:v_complexity}
                 Let $\bbX$ be a non-empty set and $h: \bbX^{m} \rightarrow \bbR$ be a kernel function with a multiplicative decomposable $\calA$ and the complexity of evaluation of $h$ is $o(n)$. Then for any $\bm{X} \in \bbX^n: n \geq m$,
                  \begin{align*}
                      \mathrm{Complexity}( \vstat[h] ) = O(n^{\operatorname{tw}(G_{\calA})+1})
                  \end{align*}

              \end{mylemma}

        \item This result is an extension of \cite{markov2008simulating}.

        \item Finding the optimal ordering (treewidth) is known to be NP-hard. \citep{arnborg1987complexity}
      
        \item In practice, heuristic algorithm such as greedy algorithm is used. It is well studied in \textbf{tensor network} community \cite{gray2021hyper}.

        \item Our \package{} use  \opteinsum{} to choose ordering.
...
\begin{frame}
\frametitle{Complexity: $U$-statistics}

\begin{figure}
    \centering
    \includegraphics[width=1.1\linewidth]{Figures_slides/quotient.png}
    \caption*{Quotient graphs induced by U-statistics with $\calA = ((1,2), (2,3),(3,4))$}
\end{figure}
...
\begin{itemize}
\setlength\itemsep{1.5em}
        \begin{mytheorem}[C., Zhang, Liu, 25]
                  \label{the:u_complexity}
                  Let $\bbX$ be a non-empty set and $h: \bbX^{m} \rightarrow \bbR$ be a kernel function with a multiplicative decomposable $\calA$ and the complexity of evaluation of $h$ is $o(n)$. Then for any $\bm{X} \in \bbX^n: n \geq m$,
                  \begin{align*}
                      \mathrm{Complexity}( \ustat[h] ) = \max_{\pi \in \partition_{m}^{\calA}}O(n^{\operatorname{tw}(G_{\calA_{\pi}})+1})
                  \end{align*}

              \end{mytheorem}

    \item  All graph can be determined by \textbf{quotient graph}, for $U$-statistic with kernel function satisfying graph $G_{\calA}$:

    \item The treewidth of quotient graph is less studied.
    
\begin{align*}
    \mathbb{V}[\pi](h) \leftrightarrow  G_{\calA_{\pi} } \leftrightarrow  G_{\calA} / \pi
\end{align*}
...
\begin{frame}
    \frametitle{Application 2: Motif Counts}
    \begin{itemize}
    \setlength\itemsep{1.5em}
        \item We compare the performance of our \package{} with  \igraph{} and \cugraph{}. \cugraph{} is developed by NVIDIA.
        \item On Erdős-Rényi graphs $G(n, p)$:
         ...
         \item  CPU parallel with 64 cores via \torcheinsum{} for \package{} compared with \igraph{}  
        \begin{itemize}
            \setlength\itemsep{1.5em}
            \item Count all types of 3-node or 4-node motifs.
        \end{itemize}
        \item GPU parallel with RTX 4090 via \torcheinsum{} for \package{} compared with \cugraph{}.   
                \begin{itemize}
            \setlength\itemsep{1.5em}
             \item Count only triangles.
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{table}[htbp]
\centering
\caption{ Runtime comparison (seconds) of exact triangle counting  using \texttt{u-stats} and \texttt{cugraph} with a single \textbf{GPU (NVIDIA RTX 4090, 24GB)}, across varying edge probability and $n = 10000$. Here "OOM" indicates Out Of Memory error}
\label{tab:triangle_counting}
\begin{tabular}{c|cc}
\hline
Edge Probability ($p$) & \textbf{\texttt{u-stats}} & \textbf{\texttt{cugraph}} \\
\hline
0.001 & 2.8014 & 0.1536 \\
0.005 & 2.9564 & 0.5664 \\
0.010 & 3.3363 & 1.0379 \\
...
\begin{frame}
    \frametitle{Application 3: Higher-Order Influence Functions}
    \begin{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Developed a \textbf{novel algorithm} for the exact computation of $U$-statistics.
        
        \item Created a Python package named \textbf{\package{}} that leverages the highly-optimized \texttt{Einsum} function within \torcheinsum{} and \numpyeinsum{} for efficient computation.
    

        \item Find an \textbf{optimal upper bound} for the computational complexity of exactly computing $U$-statistics.
    \end{itemize}
...
\frametitle{}
    ...
    \begin{itemize}\setlength\itemsep{0.8em}
        ...
        \href{https://github.com/zrq1706/U-Statistics-python}{\texttt{github.com/zrq1706/U-Statistics-python}}
        
        \item Personal website: \\
        \href{https://cxy0714.github.io/}{\texttt{https://cxy0714.github.io/}}
    \end{itemize}
